...
Public Class FrmDrugOutReg
    Private _drugDict As DataTable
    Private _inModifyStatus As Boolean = False

    Private Sub FrmDrugOutReg_Load(sender As Object, e As EventArgs) Handles Me.Load
        Me.cmbOutType.Items.AddRange(New String() {"退回至大药房", "出库至其他"})
        InitialTable()
        InitialDrugInfo()
        Reset()
    ...
    Private Function CheckParameter(ByRef errorMsg As String) As Boolean
        ...
        '进行库存数量验证
        Dim stockAmount As Long, drugManage As New DbDrugManage
        If Not drugManage.GetStockAmount(stockAmount, selRows(0).Item(DRUG_ID).ToString(), "") Then
            errorMsg = MSG_DRUG_ERROR_GET_STOCK_AMOUNT
            Return False
        End If
        If CLng(Me.tbAmount.Text) > stockAmount Then
            errorMsg = String.Format(MSG_DRUG_ERROR_STOCK_AMOUNT_NOT_INNOUGH, stockAmount)
            Return False
        End If
    ...
    Private Sub btnOk_Click(sender As Object, e As EventArgs) Handles btnOk.Click
        ...
        Dim outType = IIf(Me.cmbOutType.SelectedIndex = 0, DrugInOutType.OutToMain, DrugInOutType.OutToOther)
        If DBMEDITS_RESULT.SUCCESS = drugManage.CommitDrugOut(dataSource, LocalData.LoginUser, outType) Then
            UIMsgBox.Show(MSG_DRUG_OUT_SUCCESS)
            Reset()
            For i = Me.dgv.Rows.Count - 1 To 0 Step -1
                Me.dgv.Rows.RemoveAt(i)
            Next
