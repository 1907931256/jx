...
namespace UIGraphLib
{
	...
	#region Fields
		...
		/// <summary>
		/// protected field that stores a <see c_ref="Label" /> instance for this
		/// <see c_ref="CurveItem"/>, which is used for the <see c_ref="Legend" />
		/// label.  Use the public
		/// property <see c_ref="Label"/> to access this value.
		/// </summary>
		internal Label _label;

		/// <summary>
		/// protected field that stores the boolean value that determines whether this
		/// <see c_ref="CurveItem"/> is on the bottom X axis or the top X axis (X2).
		/// Use the public property <see c_ref="IsX2Axis"/> to access this value.
		/// </summary>
		protected bool _isX2Axis;
		/// <summary>
		/// protected field that stores the boolean value that determines whether this
		/// <see c_ref="CurveItem"/> is on the left Y axis or the right Y axis (Y2).
		/// Use the public property <see c_ref="IsY2Axis"/> to access this value.
		/// </summary>
		protected bool _isY2Axis;

		/// <summary>
		/// protected field that stores the index number of the Y Axis to which this
		/// <see c_ref="CurveItem" /> belongs.  Use the public property <see c_ref="YAxisIndex" />
		...
		/// <summary>
		/// protected field that stores the boolean value that determines whether this
		/// <see c_ref="CurveItem"/> is visible on the graph.
		...
		/// have to set <see c_ref="ZedGraph.Label.IsVisible"/> to false.
		/// </summary>
		...
		/// <summary>
		/// Protected field that stores the boolean value that determines whether this
		/// <see c_ref="CurveItem"/> is selected on the graph.
		/// Use the public property <see c_ref="IsSelected"/> to access this value.
		...
		/// <summary>
		/// Protected field that stores the boolean value that determines whether this
		/// <see c_ref="CurveItem"/> can be selected in the graph.
		/// </summary>
		protected bool _isSelectable;

		/// <summary>
		/// protected field that stores a boolean value which allows you to override the normal
		/// ordinal axis behavior.  Use the public property <see c_ref="IsOverrideOrdinal"/> to
		/// access this value.
		/// </summary>
		protected bool		_isOverrideOrdinal;
		
		/// <summary>
		/// The <see c_ref="IPointList"/> of value sets that
		/// represent this <see c_ref="CurveItem"/>.
		...
		/// (see <see c_ref="PointPairBase.Missing"/>),
		...
		/// information associated with the <see c_ref="CurveItem"/>.  ZedGraph does
	...
	#region Constructors
		/// <summary>
		/// <see c_ref="CurveItem"/> constructor the pre-specifies the curve label, the
		/// x and y data values as a <see c_ref="IPointList"/>, the curve
		/// type (Bar or Line/Symbol), the <see c_ref="Color"/>, and the
		/// <see c_ref="SymbolType"/>. Other properties of the curve are
		/// defaulted to the values in the <see c_ref="GraphPane.Default"/> class.
		/// </summary>
...
*/
		...
		/// <summary>
		/// <see c_ref="CurveItem"/> constructor that specifies the label of the CurveItem.
		/// This is the same as <c>CurveItem(label, null, null)</c>.
		/// <seealso c_ref="CurveItem( string, double[], double[] )"/>
		/// </summary>
		/// <param name="label">A string label (legend entry) for this curve</param>
	...
	#region Properties
		/// <summary>
		/// A <see c_ref="Label" /> instance that represents the <see c_ref="ZedGraph.Legend"/>
		/// entry for the this <see c_ref="CurveItem"/> object
		/// </summary>
		public Label Label
		{
			get { return _label; }
		...
		/// <summary>
		/// The <see c_ref="Line"/>/<see c_ref="Symbol"/>/<see c_ref="Bar"/> 
		/// color (FillColor for the Bar).  This is a common access to
		/// <see c_ref="ZedGraph.LineBase.Color">Line.Color</see>,
		/// <see c_ref="ZedGraph.LineBase.Color">Border.Color</see>, and
		/// <see c_ref="ZedGraph.Fill.Color">Fill.Color</see> properties for this curve.
		/// </summary>
		public Color Color
		{
			get
		...
		/// <summary>
		/// Determines whether this <see c_ref="CurveItem"/> is visible on the graph.
		/// Note that this value turns the curve display on or off, but it does not
		/// affect the display of the legend entry.  To hide the legend entry, you
		/// have to set <see c_ref="ZedGraph.Label.IsVisible"/> to false.
		/// </summary>
		...
		// Revision: JCarpenter 10/06
		/// <summary>
		/// Determines whether this <see c_ref="CurveItem"/> is selected on the graph.
		...
		// Revision: JCarpenter 10/06
		/// <summary>
		/// Determines whether this <see c_ref="CurveItem"/> can be selected in the graph.
		/// </summary>
		...
		/// <summary>
		/// Gets or sets a value that determines which X axis this <see c_ref="CurveItem"/>
		/// is assigned to.
		/// </summary>
		/// <remarks>
		/// The
		/// <see c_ref="ZedGraph.XAxis"/> is on the bottom side of the graph and the
		/// <see c_ref="ZedGraph.X2Axis"/> is on the top side.  Assignment to an axis
		/// determines the scale that is used to draw the curve on the graph.
		/// </remarks>
		/// <value>true to assign the curve to the <see c_ref="ZedGraph.X2Axis"/>,
		/// false to assign the curve to the <see c_ref="ZedGraph.XAxis"/></value>
		...
		/// <summary>
		/// Gets or sets a value that determines which Y axis this <see c_ref="CurveItem"/>
		/// is assigned to.
		/// </summary>
		/// <remarks>
		/// The
		/// <see c_ref="ZedGraph.YAxis"/> is on the left side of the graph and the
		/// <see c_ref="ZedGraph.Y2Axis"/> is on the right side.  Assignment to an axis
		/// determines the scale that is used to draw the curve on the graph.  Note that
		/// this value is used in combination with the <see c_ref="YAxisIndex" /> to determine
		/// which of the Y Axes (if there are multiples) this curve belongs to.
		/// </remarks>
		/// <value>true to assign the curve to the <see c_ref="ZedGraph.Y2Axis"/>,
		/// false to assign the curve to the <see c_ref="ZedGraph.YAxis"/></value>
		...
		/// </summary>
		/// <remarks>
		/// This value is essentially an index number into the <see c_ref="GraphPane.YAxisList" />
		/// or <see c_ref="GraphPane.Y2AxisList" />, depending on the setting of
		/// <see c_ref="IsY2Axis" />.
		/// </remarks>
		...
		/// <summary>
		/// Determines whether this <see c_ref="CurveItem"/>
		/// is a <see c_ref="BarItem"/>.
		/// </summary>
		...
		/// <see c_ref="CurveItem"/> object, which is the number of points in the
		/// <see c_ref="Points"/> data collection.
		/// </summary>
		public int NPts
		{
			get 
		...
		/// <summary>
		/// The <see c_ref="IPointList"/> of X,Y point sets that represent this
		/// <see c_ref="CurveItem"/>.
		/// </summary>
		...
		/// <summary>
		/// An accessor for the <see c_ref="PointPair"/> datum for this <see c_ref="CurveItem"/>.
		...
		/// <summary>
		/// Gets or sets the hyperlink information for this <see c_ref="CurveItem" />.
		/// </summary>
	...
	#region Rendering Methods
		/// <summary>
		/// Do all rendering associated with this <see c_ref="CurveItem"/> to the specified
		...
		/// <summary>
		/// Draw a legend key entry for this <see c_ref="CurveItem"/> at the specified location.
		/// This abstract base method passes through to <see c_ref="BarItem.DrawLegendKey"/> or
		/// <see c_ref="LineItem.DrawLegendKey"/> to do the rendering.
		/// </summary>
		/// <param name="g">
		/// A graphic device object to be drawn into.  This is normally e.Graphics from the
		/// PaintEventArgs argument to the Paint() method.
	...
	#region Utility Methods
		...
		/// <summary>
		/// Clears the points from this <see c_ref="CurveItem"/>.  This is the same
		...
		/// <summary>
		/// Removes a single point from this <see c_ref="CurveItem" />.
		/// </summary>
		/// <remarks>
		/// This method will only work if the <see c_ref="IPointList" /> instance reference
		/// at <see c_ref="Points" /> supports the <see c_ref="IPointListEdit" /> interface.
		...
		/// <summary>
		/// Get the X Axis instance (either <see c_ref="XAxis" /> or <see c_ref="X2Axis" />) to
		/// which this <see c_ref="CurveItem" /> belongs.
		/// </summary>
		/// <param name="pane">The <see c_ref="GraphPane" /> object to which this curve belongs.</param>
		/// <returns>Either a <see c_ref="XAxis" /> or <see c_ref="X2Axis" /> to which this
		/// <see c_ref="CurveItem" /> belongs.
		/// </returns>
		...
		/// </summary>
		/// <remarks>
		/// This method safely retrieves a Y Axis instance from either the <see c_ref="GraphPane.YAxisList" />
		/// or the <see c_ref="GraphPane.Y2AxisList" /> using the values of <see c_ref="YAxisIndex" /> and
		/// <see c_ref="IsY2Axis" />.  If the value of <see c_ref="YAxisIndex" /> is out of bounds, the
		/// default <see c_ref="YAxis" /> or <see c_ref="Y2Axis" /> is used.
		/// </remarks>
		/// <param name="pane">The <see c_ref="GraphPane" /> object to which this curve belongs.</param>
		/// <returns>Either a <see c_ref="YAxis" /> or <see c_ref="Y2Axis" /> to which this
		/// <see c_ref="CurveItem" /> belongs.
		/// </returns>
		...
		/// </summary>
		/// <remarks>
		/// This method safely retrieves a Y Axis index into either the <see c_ref="GraphPane.YAxisList" />
		/// or the <see c_ref="GraphPane.Y2AxisList" /> using the values of <see c_ref="YAxisIndex" /> and
		/// <see c_ref="IsY2Axis" />.  If the value of <see c_ref="YAxisIndex" /> is out of bounds, the
		/// default <see c_ref="YAxis" /> or <see c_ref="Y2Axis" /> is used, which is index zero.
		/// </remarks>
		/// <param name="pane">The <see c_ref="GraphPane" /> object to which this curve belongs.</param>
		...
		/// <seealso c_ref="ColorSymbolRotator"/>
		...
		/// <summary>
		/// Go through the list of <see c_ref="PointPair"/> data values for this <see c_ref="CurveItem"/>
		...
		/// ranging (see <see c_ref="GraphPane.IsIgnoreInitial"/>).  If true, then initial
		/// data points where the Y value is zero are not included when
		/// automatically determining the scale <see c_ref="Scale.Min"/>,
		/// <see c_ref="Scale.Max"/>, and <see c_ref="Scale.MajorStep"/> size.  All data after
		...
		/// <summary>Returns a reference to the <see c_ref="Axis"/> object that is the "base"
		/// (independent axis) from which the values are drawn. </summary>
		/// <remarks>
		/// This property is determined by the value of <see c_ref="BarSettings.Base"/> for
		/// <see c_ref="BarItem"/>, <see c_ref="ErrorBarItem"/>, and <see c_ref="HiLowBarItem"/>
		/// types.  It is always the X axis for regular <see c_ref="LineItem"/> types.
		/// Note that the <see c_ref="BarSettings.Base" /> setting can override the
		/// <see c_ref="IsY2Axis" /> and <see c_ref="YAxisIndex" /> settings for bar types
		/// (this is because all the bars that are clustered together must share the
		/// same base axis).
		/// </remarks>
		/// <seealso c_ref="BarBase"/>
		/// <seealso c_ref="ValueAxis"/>
		...
		/// This property is determined by the value of <see c_ref="BarSettings.Base"/> for
		/// <see c_ref="BarItem"/>, <see c_ref="ErrorBarItem"/>, and <see c_ref="HiLowBarItem"/>
		/// types.  It is always the Y axis for regular <see c_ref="LineItem"/> types.
		/// </remarks>
		/// <seealso c_ref="BarBase"/>
		/// <seealso c_ref="BaseAxis"/>
...
//						pane.CalcScaleFactor() );
	...
	#region Inner classes
		...
		/// <summary>
		/// Compares <see c_ref="CurveItem"/>'s based on the point value at the specified
		/// index and for the specified axis.
		/// <seealso c_ref="System.Collections.ArrayList.Sort()"/>
		/// </summary>
		public class Comparer : IComparer
		{
			...
			/// <summary>
			/// Compares two <see c_ref="CurveItem"/>s using the previously specified index value
