...
namespace UIGraphLib
{
	...
	/// <see c_ref="XAxis"/>, <see c_ref="YAxis"/>, and <see c_ref="Y2Axis"/> classes
	...
	#region Class Fields
		...
		/// <summary>
		/// private field that stores the <see c_ref="ZedGraph.Scale" /> class, which implements all the
		/// calculations and methods associated with the numeric scale for this
		/// <see c_ref="Axis" />.  See the public property <see c_ref="Scale" /> to access this class.
		/// </summary>
		internal Scale _scale;

        /// <summary>
        /// Private field that stores the <see c_ref="ZedGraph.MinorTic" /> class, which handles all
        /// the minor tic information.  See the public property <see c_ref="MinorTic" /> to access this class.
        /// </summary>
        internal MinorTic _minorTic;
        /// <summary>
        /// Private field that stores the <see c_ref="ZedGraph.MajorTic" /> class, which handles all
        /// the major tic information.  See the public property <see c_ref="MajorTic" /> to access this class.
        /// </summary>
        internal MajorTic _majorTic;

        /// <summary>
        /// Private field that stores the <see c_ref="ZedGraph.MajorGrid" /> class, which handles all
        /// the major grid information.  See the public property <see c_ref="MajorGrid" /> to access this class.
        /// </summary>
        internal MajorGrid	_majorGrid;
        /// <summary>
        /// Private field that stores the <see c_ref="ZedGraph.MinorGrid" /> class, which handles all
        /// the minor grid information.  See the public property <see c_ref="MinorGrid" /> to access this class.
        /// </summary>
        internal MinorGrid _minorGrid;

        /// <summary> Private fields for the <see c_ref="Axis"/> scale rendering properties.
        /// Use the public properties <see c_ref="Cross"/> and <see c_ref="ZedGraph.Scale.BaseTic"/>
        /// for access to these values.
        /// </summary>
        internal double _cross;

        /// <summary> Private field for the <see c_ref="Axis"/> automatic cross position mode.
        /// Use the public property <see c_ref="CrossAuto"/> for access to this value.
        /// </summary>
        internal bool _crossAuto;

        /// <summary> Private fields for the <see c_ref="Axis"/> attributes.
        /// Use the public properties <see c_ref="IsVisible"/>, <see c_ref="IsAxisSegmentVisible"/>
        /// for access to these values.
        /// </summary>
        protected bool _isVisible, _isAxisSegmentVisible;

        /// <summary> Private field for the <see c_ref="Axis"/> title string.
        /// Use the public property <see c_ref="Title"/> for access to this value.
        /// </summary>
        protected AxisLabel _title;

        /// <summary>
        /// A tag object for use by the user.  This can be used to store additional
        /// information associated with the <see c_ref="Axis"/>.  ZedGraph does
        /// not use this value for any purpose.
        /// </summary>
        /// <remarks>
        /// Note that, if you are going to Serialize ZedGraph data, then any type
        /// that you store in <see c_ref="Tag"/> must be a serializable type (or
        /// it will cause an exception).
        /// </remarks>
        public object Tag;

		/// <summary> Private field for the <see c_ref="Axis"/> drawing dimensions.
		/// Use the public property <see c_ref="AxisGap"/>
		/// for access to these values. </summary>
		private float	_axisGap;

		/// <summary>
		/// Private field for the <see c_ref="Axis"/> minimum allowable space allocation.
		/// Use the public property <see c_ref="MinSpace"/> to access this value.
		/// </summary>
		/// <seealso c_ref="Default.MinSpace"/>
		private float _minSpace;

		/// <summary> Private fields for the <see c_ref="Axis"/> colors.
		/// Use the public property <see c_ref="Color"/> for access to this values.
		/// </summary>
		private Color _color;

		/// <summary>
		/// Temporary values for axis space calculations (see <see c_ref="CalcSpace" />).
		/// </summary>
	...
	#region Events
		...
		/// formatted</param>
		/// <param name="axis">The <see c_ref="Scale" /> of interest.</param>
		...
		/// </summary>
		/// <param name="axis">The <see c_ref="Axis" /> of interest.</param>
		/// <returns>
		/// A string value representing the label, or null if the ZedGraph should go ahead
	...
	#region Defaults
		...
		/// default property values for the <see c_ref="Axis"/> class.
		/// </summary>
		public struct Default
		{
			/// <summary>
			/// The default size for the gap between multiple axes
			/// (<see c_ref="Axis.AxisGap"/> property). Units are in points (1/72 inch).
			/// </summary>
			...
			/// <summary>
			/// The default font family for the <see c_ref="Axis"/> <see c_ref="Title" /> text
			/// font specification <see c_ref="FontSpec"/>
			/// (<see c_ref="FontSpec.Family"/> property).
			/// </summary>
			public static string TitleFontFamily = "Arial";
			/// <summary>
			/// The default font size for the <see c_ref="Axis"/> <see c_ref="Title" /> text
			/// font specification <see c_ref="FontSpec"/>
			/// (<see c_ref="FontSpec.Size"/> property).  Units are
			/// in points (1/72 inch).
			/// </summary>
			public static float TitleFontSize = 14;
			/// <summary>
			/// The default font color for the <see c_ref="Axis"/> <see c_ref="Title" /> text
			/// font specification <see c_ref="FontSpec"/>
			/// (<see c_ref="FontSpec.FontColor"/> property).
			/// </summary>
			public static Color TitleFontColor = Color.Black;
			/// <summary>
			/// The default font bold mode for the <see c_ref="Axis"/> <see c_ref="Title" /> text
			/// font specification <see c_ref="FontSpec"/>
			/// (<see c_ref="FontSpec.IsBold"/> property). true
			/// for a bold typeface, false otherwise.
			/// </summary>
			public static bool TitleFontBold = true;
			/// <summary>
			/// The default font italic mode for the <see c_ref="Axis"/> <see c_ref="Title" /> text
			/// font specification <see c_ref="FontSpec"/>
			/// (<see c_ref="FontSpec.IsItalic"/> property). true
			/// for an italic typeface, false otherwise.
			/// </summary>
			public static bool TitleFontItalic = false;
			/// <summary>
			/// The default font underline mode for the <see c_ref="Axis"/> <see c_ref="Title" /> text
			/// font specification <see c_ref="FontSpec"/>
			/// (<see c_ref="FontSpec.IsUnderline"/> property). true
			/// for an underlined typeface, false otherwise.
			/// </summary>
			public static bool TitleFontUnderline = false;
            /// <summary>
            /// The default color for filling in the <see c_ref="Title" /> text background
            /// (see <see c_ref="ZedGraph.Fill.Color"/> property).
            /// </summary>
            public static Color TitleFillColor = Color.White;
            /// <summary>
            /// The default custom brush for filling in the <see c_ref="Title" /> text background
            /// (see <see c_ref="ZedGraph.Fill.Brush"/> property).
            /// </summary>
            public static Brush TitleFillBrush = null;
            /// <summary>
            /// The default fill mode for filling in the <see c_ref="Title" /> text background
            /// (see <see c_ref="ZedGraph.Fill.Type"/> property).
            /// </summary>
            public static FillType TitleFillType = FillType.None;

			/// <summary>
			/// The default color for the <see c_ref="Axis"/> itself
			/// (<see c_ref="Axis.Color"/> property).  This color only affects the
			/// the axis border.
			/// </summary>
			public static Color BorderColor = Color.Black;
			/// <summary>
			/// The default value for <see c_ref="Axis.IsAxisSegmentVisible"/>, which determines
			/// whether or not the scale segment itself is visible
			/// </summary>
			public static bool IsAxisSegmentVisible = true;

			/// <summary>
			/// The default setting for the <see c_ref="Axis"/> scale axis type
			/// (<see c_ref="Axis.Type"/> property).  This value is set as per
			/// the <see c_ref="AxisType"/> enumeration
			/// </summary>
			...
			/// points (1/72 inch) and scaled according to <see c_ref="PaneBase.CalcScaleFactor"/> for the
			/// <see c_ref="GraphPane"/>, determines the minimum amount of space an axis must
			/// have between the <see c_ref="Chart.Rect"/> and the
			/// <see c_ref="PaneBase.Rect"/>.  This minimum space
			/// applies whether <see c_ref="Axis.IsVisible"/> is true or false.
			/// </summary>
	...
	#region Constructors
		...
		/// <summary>
		/// Constructor for <see c_ref="Axis"/> that sets all axis properties
		/// to default values as defined in the <see c_ref="Default"/> class,
		/// except for the <see c_ref="Title"/>.
		/// </summary>
	...
	#region Scale Properties
		...
		/// <summary>
		/// Gets the <see c_ref="Scale" /> instance associated with this <see c_ref="Axis" />.
		/// </summary>
		public Scale Scale
		{
			get { return _scale; }
        ...
        /// automatically based on the state of <see c_ref="CrossAuto"/>.  If
        /// this value is set manually, then <see c_ref="CrossAuto"/> will
        ...
        /// <seealso c_ref="ZedGraph.Scale.Min"/>
        /// <seealso c_ref="ZedGraph.Scale.Max"/>
        /// <seealso c_ref="ZedGraph.Scale.MajorStep"/>
        /// <seealso c_ref="CrossAuto"/>
        public double Cross
		{
			get { return _cross; }
        ...
        /// <summary>
        /// Gets or sets a value that determines whether or not the <see c_ref="Cross"/> value
        /// is set automatically.
        /// </summary>
        /// <value>Set to true to have ZedGraph put the axis in the default location, or false
        /// to specify the axis location manually with a <see c_ref="Cross"/> value.</value>
        /// <seealso c_ref="ZedGraph.Scale.Min"/>
        /// <seealso c_ref="ZedGraph.Scale.Max"/>
        /// <seealso c_ref="ZedGraph.Scale.MajorStep"/>
        /// <seealso c_ref="Cross"/>
		...
		/// points (1/72 inch) and scaled according to <see c_ref="PaneBase.CalcScaleFactor"/>
		/// for the <see c_ref="GraphPane"/>, determines the minimum amount of space
		/// an axis must have between the <see c_ref="Chart.Rect">Chart.Rect</see> and the
		/// <see c_ref="PaneBase.Rect">GraphPane.Rect</see>.  This minimum space
		/// applies whether <see c_ref="IsVisible"/> is true or false.
		/// </remarks>
	...
	#region Tic Properties
		...
		/// <summary>
		/// The color to use for drawing this <see c_ref="Axis"/>.
		/// </summary>
		/// <remarks>
		/// This affects only the axis segment (see <see c_ref="IsAxisSegmentVisible" />),
		/// since the <see c_ref="Title"/>,
		/// <see c_ref="Scale"/>, <see c_ref="MajorTic" />, <see c_ref="MinorTic" />,
		/// <see c_ref="MajorGrid" />, and <see c_ref="MinorGrid" />
		/// all have their own color specification.
		/// </remarks>
		/// <value> The color is defined using the
		/// <see c_ref="System.Drawing.Color"/> class</value>
		/// <seealso c_ref="Default.Color"/>.
        ...
        /// <summary>
        /// Gets a reference to the <see c_ref="ZedGraph.MajorTic" /> class instance
        /// for this <see c_ref="Axis" />.  This class stores all the major tic settings.
        /// </summary>
        ...
        /// <summary>
        /// Gets a reference to the <see c_ref="ZedGraph.MinorTic" /> class instance
        /// for this <see c_ref="Axis" />.  This class stores all the minor tic settings.
        /// </summary>
	...
	#region Grid Properties
		...
		/// <summary>
		/// Gets a reference to the <see c_ref="MajorGrid" /> class that contains the properties
		/// of the major grid.
		/// </summary>
		...
		/// <summary>
		/// Gets a reference to the <see c_ref="MinorGrid" /> class that contains the properties
		/// of the minor grid.
		/// </summary>
        ...
        /// <summary>
        /// This property determines whether or not the <see c_ref="Axis"/> is shown.
        /// </summary>
        /// <remarks>
        /// Note that even if
        ...
        /// <seealso c_ref="ZedGraph.Scale.IsVisible"/>.
        /// <seealso c_ref="XAxis.Default.IsVisible"/>.
        /// <seealso c_ref="YAxis.Default.IsVisible"/>.
        /// <seealso c_ref="Y2Axis.Default.IsVisible"/>.
		...
		/// the Axis segment is overlain by the Axis border (see <see c_ref="Chart.Border"/>).
		/// However, when the border is not visible, or when <see c_ref="Axis.CrossAuto"/> is set to
        ...
        /// <summary>
        /// Gets or sets the <see c_ref="AxisType"/> for this <see c_ref="Axis"/>.
        /// </summary>
        /// <remarks>
        /// The type can be either <see c_ref="AxisType.Linear"/>,
        /// <see c_ref="AxisType.Log"/>, <see c_ref="AxisType.Date"/>,
        /// or <see c_ref="AxisType.Text"/>.
        /// </remarks>
        /// <seealso c_ref="ZedGraph.Scale.IsLog"/>
        /// <seealso c_ref="ZedGraph.Scale.IsText"/>
        /// <seealso c_ref="ZedGraph.Scale.IsOrdinal"/>
        /// <seealso c_ref="ZedGraph.Scale.IsDate"/>
        /// <seealso c_ref="ZedGraph.Scale.IsReverse"/>
	...
	#region Label Properties
		...
		/// <summary>
		/// Gets or sets the <see c_ref="Label" /> class that contains the title of this
		/// <see c_ref="Axis"/>.
		/// </summary>
		...
		/// <see c_ref="Label.IsVisible"/> property is set to true.  If the Title text is empty,
		...
		/// <summary>
		/// The size of the gap between multiple axes (see <see c_ref="GraphPane.YAxisList" /> and
		/// <see c_ref="GraphPane.Y2AxisList" />).
		/// </summary>
		/// <remarks>
		/// This size will be scaled
		/// according to the <see c_ref="PaneBase.CalcScaleFactor"/> for the
		/// <see c_ref="GraphPane"/>
		/// </remarks>
		/// <value>The axis gap is measured in points (1/72 inch)</value>
		/// <seealso c_ref="Default.AxisGap"/>.
		...
		/// <summary>
		/// Do all rendering associated with this <see c_ref="Axis"/> to the specified
		/// <see c_ref="Graphics"/> device.
		/// </summary>
		/// <remarks>
		/// This method is normally only
		/// called by the Draw method of the parent <see c_ref="GraphPane"/> object.
		/// </remarks>
		/// <param name="g">
		/// A graphic device object to be drawn into.  This is normally e.Graphics from the
		/// PaintEventArgs argument to the Paint() method.
		...
		/// <summary>
		/// This method will set the <see c_ref="MinSpace"/> property for this <see c_ref="Axis"/>
		/// using the currently required space multiplied by a fraction (<paramref>bufferFraction</paramref>).
		/// </summary>
		/// <remarks>
		/// The currently required space is calculated using <see c_ref="CalcSpace"/>, and is
		...
		/// <param name="isGrowOnly">If true, then this method will only modify the <see c_ref="MinSpace"/>
		...
		/// <summary>
		/// Setup the Transform Matrix to handle drawing of this <see c_ref="Axis"/>
		/// </summary>
		/// <param name="g">
		/// A graphic device object to be drawn into.  This is normally e.Graphics from the
		/// PaintEventArgs argument to the Paint() method.
		...
		/// location to the value specified by <see c_ref="Cross"/>.
		/// </summary>
		/// <param name="pane">
		/// A reference to the <see c_ref="GraphPane"/> object that is the parent or
		/// owner of this object.
        ...
        /// <see c_ref="Cross" />, <see c_ref="CrossAuto" />, and <see c_ref="ZedGraph.Scale.IsReverse" />.
        /// </summary>
        /// <remarks>
        /// If the value of <see c_ref="Cross" /> lies outside the axis range, it is
		...
		/// <see c_ref="Cross" />.  This function will always return false if
		/// <see c_ref="CrossAuto" /> is true.
		/// </summary>
		...
		/// <summary>
		/// Calculate the space required (pixels) for this <see c_ref="Axis"/> object.
		/// </summary>
		/// <remarks>
		/// This is the total space (vertical space for the X axis, horizontal space for
		/// the Y axes) required to contain the axis.  If <see c_ref="Cross" /> is zero, then
		/// this space will be the space required between the <see c_ref="Chart.Rect" /> and
		/// the <see c_ref="PaneBase.Rect" />.  This method sets the internal values of
		/// <see c_ref="_tmpSpace" /> for use by the <see c_ref="GraphPane.CalcChartRect(Graphics)" />
		/// method.
		/// </remarks>
		/// <param name="g">
		/// A graphic device object to be drawn into.  This is normally e.Graphics from the
		...
		/// <summary>
		/// Draw the title for this <see c_ref="Axis"/>.
		/// </summary>
		...
		/// </summary>
		/// <remarks>
		/// This method properly accounts for <see c_ref="ZedGraph.Scale.IsLog"/>,
		/// <see c_ref="ZedGraph.Scale.IsText"/>,
		...
		/// (<see c_ref="ZedGraph.Scale.IsLog"/>)
		/// and text (<see c_ref="ZedGraph.Scale.IsText"/>) type axes.
		/// </param>
