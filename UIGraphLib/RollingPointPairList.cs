...
{
	/// <summary>
	/// A class that provides a rolling list of <see c_ref="PointPair" /> objects.
	...
	/// accessing via <see c_ref="IPointList" />, the <see c_ref="PointPair" /> objects are
	/// seen in the order in which they were enqeued.
	///
	/// RollingPointPairList supports data editing through the <see c_ref="IPointListEdit" />
	/// interface.
	/// 
	...
	#region Properties
		...
		/// <summary>
		/// Gets or sets the <see c_ref="PointPair" /> at the specified index in the buffer.
		/// </summary>
		/// <remarks>
		/// Index must be within the current size of the buffer, e.g., the set
		/// method will not expand the buffer even if <see c_ref="Capacity" /> is available
		/// </remarks>
	...
	#region Public Methods
		...
		/// <summary>
		/// Clear the buffer of all <see c_ref="PointPair"/> objects.
		/// Note that the <see c_ref="Capacity" /> remains unchanged.
		/// </summary>
		public void Clear()
		{
			_headIdx = _tailIdx = -1;
		...
		/// <summary>
		/// Add an <see c_ref="IPointList"/> object to the head of the queue.
		/// </summary>
		/// <param name="pointList">A reference to the <see c_ref="IPointList"/> object to
		/// be added</param>
		...
		/// </summary>
		/// <returns>The removed item. Throws an <see c_ref="InvalidOperationException" />
		/// if the buffer was empty. 
		/// Check the buffer's length (<see c_ref="Count" />) or the <see c_ref="IsEmpty" />
		...
		/// <summary>
		/// Remove the <see c_ref="PointPair" /> at the specified index
		/// </summary>
		/// <remarks>
		...
		/// Throws an <see c_ref="ArgumentOutOfRangeException" /> if index is less than
		/// zero or greater than or equal to <see c_ref="Count" />
		/// </param>
		...
		/// <summary>
		/// Remove a range of <see c_ref="PointPair" /> objects starting at the specified index
		/// </summary>
		/// <remarks>
		/// All items in the queue that lie after <paramref name="index"/> will
		...
		/// <see c_ref="ArgumentOutOfRangeException" /> if <paramref name="count" /> is less than zero
		...
		/// <summary>
		/// Peek at the <see c_ref="PointPair" /> item at the head of the queue.
		/// </summary>
		/// <returns>The <see c_ref="PointPair" /> item at the head of the queue.
		/// Throws an <see c_ref="InvalidOperationException" /> if the buffer was empty.
		/// </returns>
	...
	#region Auxilliary Methods
		...
		/// In this way, each PointPair position in the rolling list will only be allocated one time.
		/// To truly be memory efficient, the <see c_ref="Remove" />, <see c_ref="RemoveAt" />,
		/// and <see c_ref="Pop" /> methods should be avoided.
		/// </remarks>
		/// <param name="x">The X value</param>
		/// <param name="y">The Y value</param>
		...
		/// <summary>
		/// Add a set of points to the <see c_ref="RollingPointPairList"/>
