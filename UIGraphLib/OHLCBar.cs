...
{
	/// <summary>
	/// This class handles the drawing of the curve <see c_ref="OHLCBar"/> objects.
	/// </summary>
	/// 
	...
	#region Fields
		...
		/// <summary>
		/// Private field that stores the visibility of the <see c_ref="OHLCBar"/> open and
		/// close line segments ("wings").  Use the public
		/// property <see c_ref="IsOpenCloseVisible"/> to access this value.  If this value is
		...
		/// segments.  Use the public property <see c_ref="Size"/> to access this value.
		/// </summary>
		protected float _size;

		/// <summary>
		/// Private field that determines if the <see c_ref="Size" /> property will be
		/// calculated automatically based on the minimum axis scale step size between
		/// bars.  Use the public property <see c_ref="IsAutoSize" /> to access this value.
		/// </summary>
	...
	#region Defaults
		...
		/// default property values for the <see c_ref="ZedGraph.OHLCBar"/> class.
		/// </summary>
		new public struct Default
		{
			// Default Symbol properties
			/// <summary>
			/// The default width for the candlesticks (see <see c_ref="OHLCBar.Size" />),
			/// in units of points.
			/// </summary>
			public static float Size = 12;
			/// <summary>
			/// The default display mode for symbols (<see c_ref="OHLCBar.IsOpenCloseVisible"/> property).
			/// true to display symbols, false to hide them.
			/// </summary>
			public static bool IsOpenCloseVisible = true;

			/// <summary>
			/// The default value for the <see c_ref="ZedGraph.OHLCBar.IsAutoSize" /> property.
			/// </summary>
	...
	#region Properties
		...
		/// <summary>
		/// Gets or sets a property that shows or hides the <see c_ref="OHLCBar"/> open/close "wings".
		/// </summary>
		/// <value>true to show the CandleStick wings, false to hide them</value>
		/// <seealso c_ref="Default.IsOpenCloseVisible"/>
		...
		/// segments ("wings") of the <see c_ref="OHLCBar" /> items. Units are points.
		/// </summary>
		/// <remarks>The size of the candlesticks can be set by this value, which
		/// is then scaled according to the scaleFactor (see
		/// <see c_ref="PaneBase.CalcScaleFactor"/>).  Alternatively,
		/// if <see c_ref="IsAutoSize"/> is true, the bar width will
		/// be set according to the maximum available cluster width less
		/// the cluster gap (see <see c_ref="BarSettings.GetClusterWidth"/>
		/// and <see c_ref="BarSettings.MinClusterGap"/>).  That is, if
		/// <see c_ref="IsAutoSize"/> is true, then the value of
		/// <see c_ref="Size"/> will be ignored.  If you modify the value of Size,
		/// then <see c_ref="IsAutoSize" /> will be automatically set to false.
		/// </remarks>
		...
		/// <summary>
		/// Gets or sets a value that determines if the <see c_ref="Size" /> property will be
		/// calculated automatically based on the minimum axis scale step size between
		/// bars.
		/// </summary>
	...
	#region Constructors
		...
		/// <summary>
		/// Default constructor that sets all <see c_ref="OHLCBar"/> properties to
		...
		/// <see c_ref="OHLCBar"/> properties to default
	...
	#region Rendering Methods
		...
		/// <summary>
		/// Draw the <see c_ref="OHLCBar"/> to the specified <see c_ref="Graphics"/>
		...
		/// <see c_ref="OHLCBar"/> is the X axis.  True for an <see c_ref="XAxis"/> base,
		...
		/// <see c_ref="LineBase.Width"/> for this <see c_ref="OHLCBar"/></param>
...
//				using ( Pen pen = new Pen( _color, _penWidth ) )
		...
		/// <see c_ref="Size"/> and <see c_ref="IsAutoSize"/>.
		/// </summary>
		/// <param name="pane">The parent <see c_ref="GraphPane"/> object.</param>
		/// <param name="baseAxis">The <see c_ref="Axis"/> object that
