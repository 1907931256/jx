...
namespace UIGraphLib
{
	...
	/// parent class for <see c_ref="MasterPane"/> and <see c_ref="GraphPane"/>.
	/// </summary>
	/// 
	...
	#region Fields
		...
		/// Use the public property <see c_ref="Rect"/> to access this value.
		/// </summary>
		protected RectangleF	_rect;
		
		/// <summary>Private field that holds the main title of the pane.  Use the
		/// public property <see c_ref="Title"/> to access this value.
		/// </summary>
		protected GapLabel _title;
		
		/// <summary>Private field instance of the <see c_ref="ZedGraph.Legend"/> class.  Use the
		/// public property <see c_ref="PaneBase.Legend"/> to access this class.</summary>
		protected Legend _legend;

		/// <summary>
		/// Private field that stores the user-defined tag for this <see c_ref="PaneBase"/>.  This tag
		/// can be any user-defined value.  If it is a <see c_ref="String"/> type, it can be used as
		/// a parameter to the <see c_ref="PaneList.IndexOfTag"/> method.  Use the public property
		/// <see c_ref="Tag"/> to access this value.
		/// </summary>
		protected object _tag;

		/// <summary>
		/// private field to store the margin values for this <see c_ref="PaneBase" />. Use the
		/// public property <see c_ref="Margin" /> to access this property.
		/// </summary>
		...
		/// Use the public property <see c_ref="IsFontsScaled"/> to access this value. </summary>
		/// <seealso c_ref="CalcScaleFactor"/>
		/// <seealso c_ref="IsPenWidthScaled"/>
		protected bool _isFontsScaled;
		/// <summary>
		/// Private field that controls whether or not pen widths are scaled according to the
		/// size of the graph.  This value is only applicable if <see c_ref="IsFontsScaled"/>
		/// is true.  If <see c_ref="IsFontsScaled"/> is false, then no scaling will be done,
		/// regardless of the value of <see c_ref="IsPenWidthScaled"/>.
		/// </summary>
		/// <value>true to scale the pen widths according to the size of the graph,
		/// false otherwise.</value>
		/// <seealso c_ref="IsFontsScaled"/>
		/// <seealso c_ref="CalcScaleFactor"/>
		protected bool _isPenWidthScaled;

		/// <summary>
		/// Private field that stores the <see c_ref="ZedGraph.Fill"/> data for the
		/// <see c_ref="Rect"/> background.  Use the public property <see c_ref="Fill"/> to
		/// access this value.
		/// </summary>
		protected Fill	_fill;
		/// <summary>
		/// Private field that stores the <see c_ref="ZedGraph.Border"/> data for the
		/// <see c_ref="Rect"/> border.  Use the public property <see c_ref="Border"/> to
		/// access this value.
		/// </summary>
		protected Border _border;

		/// <summary>Private field instance of the <see c_ref="ZedGraph.GraphObjList"/> class.  Use the
		/// public property <see c_ref="GraphObjList"/> to access this class.</summary>
		...
		/// Use the public property <see c_ref="BaseDimension"/> to access this value. </summary>
		/// <seealso c_ref="_isFontsScaled"/>
		/// <seealso c_ref="CalcScaleFactor"/>
	...
	#region Defaults
		/// <summary>
		/// A simple struct that defines the default property values for the <see c_ref="PaneBase"/> class.
		/// </summary>
		public struct Default
		{
			...
			/// (<see c_ref="PaneBase.Title"/> <see c_ref="Label.IsVisible" /> property).  true to
			...
			/// (<see c_ref="PaneBase.Title"/> property).
			/// </summary>
			public static string FontFamily = "Arial";
			/// <summary>
			/// The default font size (points) for the
			/// <see c_ref="PaneBase.Title"/> (<see c_ref="ZedGraph.FontSpec.Size"/> property).
			/// </summary>
			public static float FontSize = 16;
			/// <summary>
			/// The default font color for the
			/// <see c_ref="PaneBase.Title"/>
			...
			/// <summary>
			/// The default border mode for the <see c_ref="PaneBase"/>.
			/// (<see c_ref="PaneBase.Border"/> property). true
			/// to draw a border around the <see c_ref="PaneBase.Rect"/>,
			/// false otherwise.
			/// </summary>
			public static bool IsBorderVisible = true;
			/// <summary>
			/// The default color for the <see c_ref="PaneBase"/> border.
			/// (<see c_ref="PaneBase.Border"/> property). 
			/// </summary>
			public static Color BorderColor = Color.Black;
			/// <summary>
			/// The default color for the <see c_ref="PaneBase.Rect"/> background.
			/// (<see c_ref="PaneBase.Fill"/> property). 
			/// </summary>
			public static Color FillColor = Color.White;

			/// <summary>
			/// The default pen width for the <see c_ref="PaneBase"/> border.
			/// (<see c_ref="PaneBase.Border"/> property).  Units are in points (1/72 inch).
			/// </summary>
			public static float BorderPenWidth = 1;

			/// <summary>
			/// The default dimension of the <see c_ref="PaneBase.Rect"/>, which
			/// defines a normal sized plot.  This dimension is used to scale the
			/// fonts, symbols, etc. according to the actual size of the
			/// <see c_ref="PaneBase.Rect"/>.
			/// </summary>
			/// <seealso c_ref="PaneBase.CalcScaleFactor"/>
			public static float BaseDimension = 8.0F;
			
			/// <summary>
			/// The default setting for the <see c_ref="PaneBase.IsPenWidthScaled"/> option.
			/// true to have all pen widths scaled according to <see c_ref="PaneBase.BaseDimension"/>,
			/// false otherwise.
			/// </summary>
			/// <seealso c_ref="PaneBase.CalcScaleFactor"/>
			public static bool IsPenWidthScaled = false;
			/// <summary>
			/// The default setting for the <see c_ref="PaneBase.IsFontsScaled"/> option.
			/// true to have all fonts scaled according to <see c_ref="PaneBase.BaseDimension"/>,
			/// false otherwise.
			/// </summary>
			/// <seealso c_ref="PaneBase.CalcScaleFactor"/>
			...
			/// <summary>
			/// The default value for the <see c_ref="PaneBase.TitleGap" /> property, expressed as
			/// a fraction of the scaled <see c_ref="Title" /> character height.
			/// </summary>
	...
	#region Properties
		...
		/// <summary>
		/// Accesses the <see c_ref="Legend"/> for this <see c_ref="PaneBase"/>
		/// </summary>
		...
		/// <summary>
		/// Gets the <see c_ref="Label" /> instance that contains the text and attributes of the title.
		...
		/// <summary>
		/// Gets or sets the user-defined tag for this <see c_ref="PaneBase"/>.  This tag
		/// can be any user-defined value.  If it is a <see c_ref="String"/> type, it can be used as
		/// a parameter to the <see c_ref="PaneList.IndexOfTag"/> method.
		/// </summary>
		/// <remarks>
		...
		/// border around the <see c_ref="Rect"/>
		/// </summary>
		...
		/// <summary>
		/// Gets or sets the <see c_ref="ZedGraph.Fill"/> data for the
		/// filling the background of the <see c_ref="Rect"/>.
		/// </summary>
		public Fill	Fill
		{
			get { return _fill; }
		...
		/// <summary>
		/// Gets or sets the list of <see c_ref="GraphObj"/> items for this <see c_ref="GraphPane"/>
		/// </summary>
		/// <value>A reference to a <see c_ref="GraphObjList"/> collection object</value>
		...
		/// <summary>
		/// Gets or sets the <see c_ref="ZedGraph.Margin" /> instance that controls the space between
		/// the edge of the <see c_ref="PaneBase.Rect" /> and the rendered content of the graph.
		/// </summary>
		...
		/// <see c_ref="Title"/> size is 14 points.  Then the pane title font
		/// will be 14 points high when the <see c_ref="Rect"/> is approximately 8.0
		...
		/// </summary>
		/// <value>The base dimension reference for the <see c_ref="Rect"/>, in inches</value>
		/// <seealso c_ref="Default.BaseDimension"/>
		/// <seealso c_ref="IsFontsScaled"/>
		/// <seealso c_ref="CalcScaleFactor"/>
		...
		/// <see c_ref="Title" /> character height.
		/// </summary>
		...
		/// the size of the <see c_ref="Rect"/> and the <see c_ref="BaseDimension"/>.  If this
		...
		/// <see c_ref="PaneBase"/>.
		/// </summary>
		/// <remarks>This value is only applicable if <see c_ref="IsFontsScaled"/>
		/// is true.  If <see c_ref="IsFontsScaled"/> is false, then no scaling will be done,
		/// regardless of the value of <see c_ref="IsPenWidthScaled"/>.  Note that scaling the pen
		...
		/// <see c_ref="GetImage(int,int,float)"/>) where it is desirable to have the pen width
	...
	#region Constructors
		...
		/// <summary>
		/// Default constructor for the <see c_ref="PaneBase"/> class.  Leaves the <see c_ref="Rect"/> empty.
		/// </summary>
	...
	#region Methods
		...
		/// <summary>
		/// Do all rendering associated with this <see c_ref="PaneBase"/> to the specified
		/// <see c_ref="Graphics"/> device.  This abstract method is implemented by the child
		/// classes.
		/// </summary>
		/// <param name="g">
		...
		/// <summary>
		/// Calculate the client area rectangle based on the <see c_ref="PaneBase.Rect"/>.
		/// </summary>
		/// <remarks>The client rectangle is the actual area available for <see c_ref="GraphPane"/>
		/// or <see c_ref="MasterPane"/> items after taking out space for the margins and the title.
		/// This method does not take out the area required for the <see c_ref="PaneBase.Legend"/>.
		/// To do so, you must separately call <see c_ref="ZedGraph.Legend.CalcRect"/>.
		/// </remarks>
		/// <param name="g">
		/// A graphic device object to be drawn into.  This is normally e.Graphics from the
		/// PaintEventArgs argument to the Paint() method.
		...
		/// The scaling factor for the features of the graph based on the <see c_ref="PaneBase.Default.BaseDimension"/>.  This
		...
		/// <summary>
		/// Draw the border _border around the <see c_ref="Rect"/> area.
		/// </summary>
		/// <param name="g">
		/// A graphic device object to be drawn into.  This is normally e.Graphics from the
		/// PaintEventArgs argument to the Paint() method.
		/// </param>
		/// <param name="scaleFactor">
		/// The scaling factor for the features of the graph based on the <see c_ref="BaseDimension"/>.  This
		/// scaling factor is calculated by the <see c_ref="CalcScaleFactor"/> method.  The scale factor
		/// represents a linear multiple to be applied to font sizes, symbol sizes, etc.
		/// </param>		
		...
		/// <summary>
		/// Draw the <see c_ref="Title"/> on the graph, centered at the top of the pane.
		/// </summary>
		/// <param name="g">
		/// A graphic device object to be drawn into.  This is normally e.Graphics from the
		/// PaintEventArgs argument to the Paint() method.
		...
		/// <summary>
		/// Change the size of the <see c_ref="Rect"/>.  Override this method to handle resizing the contents
		/// as required.
		/// </summary>
		/// <param name="g">
		/// A graphic device object to be drawn into.  This is normally e.Graphics from the
		/// PaintEventArgs argument to the Paint() method.
		/// </param>
		/// <param name="rect">The new size for the <see c_ref="Rect"/>.</param>
		...
		/// <summary>
		/// Calculate the scaling factor based on the ratio of the current <see c_ref="Rect"/> dimensions and
		/// the <see c_ref="Default.BaseDimension"/>.
		/// </summary>
		/// <remarks>This scaling factor is used to proportionally scale the
		/// features of the <see c_ref="MasterPane"/> so that small graphs don't have huge fonts, and vice versa.
		...
		/// is a dimension based on points (1/72nd inch) assuming that the <see c_ref="Rect"/> size
		/// matches the <see c_ref="Default.BaseDimension"/>.
		...
		/// </remarks>
		/// <returns>
		/// A <see c_ref="Single"/> value representing the scaling factor to use for the rendering calculations.
		/// </returns>
		/// <seealso c_ref="PaneBase.BaseDimension"/>
		...
		/// setting of the <see c_ref="IsPenWidthScaled"/> property of the pane.
		/// </summary>
		...
		/// </summary>
		/// <value>A <see c_ref="Bitmap"/> object rendered with the current graph.</value>
		/// <seealso c_ref="GetImage(int,int,float)"/>
		/// <seealso c_ref="GetMetafile()"/>
		/// <seealso c_ref="GetMetafile(int,int)"/>
		...
		/// <summary>
		/// Setup a <see c_ref="Graphics" /> instance with appropriate antialias settings.
		/// </summary>
		/// <remarks>
		...
		/// disposed.</remarks>
		/// <param name="g">An existing <see c_ref="Graphics" /> instance</param>
		...
		/// <see c_ref="IsFontsScaled" /> are affected by the size of the expected image.
		/// </remarks>
