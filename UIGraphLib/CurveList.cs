...
{
	/// <summary>
	/// A collection class containing a list of <see c_ref="CurveItem"/> objects
	...
	#region Properties
		...
		/// type <see c_ref="BarItem"/>. This does not include <see c_ref="HiLowBarItem" /> or
		/// <see c_ref="ErrorBarItem" /> types.
		/// </summary>
		public int NumBars
		{
			get
		...
		/// potentially "clusterable", which includes <see c_ref="BarItem"/> and
		/// <see c_ref="HiLowBarItem" /> types. This does not include <see c_ref="ErrorBarItem" />,
		/// <see c_ref="OHLCBarItem" />, <see c_ref="JapaneseCandleStickItem" />, etc. types.
		/// </summary>
		...
		/// <summary>
		/// Read only property that determines if all items in the <see c_ref="CurveList"/> are
		/// Pies.
		/// </summary>
		...
		/// <summary>
		/// Determine if there is any data in any of the <see c_ref="CurveItem"/>
		/// objects for this graph.  This method does not verify valid data, it
		/// only checks to see if <see c_ref="CurveItem.NPts"/> > 0.
		/// </summary>
	...
	#region IEnumerable Methods
		...
		//CJBL
		/// <summary>
		/// Iterate backwards through the <see c_ref="CurveList" /> items.
		/// </summary>
		...
		/// <summary>
		/// Iterate forward through the <see c_ref="CurveList" /> items.
		/// </summary>
...
/*
		/// <summary>
		/// Indexer to access the specified <see c_ref="CurveItem"/> object by
		/// its ordinal position in the list.
		/// </summary>
		/// <param name="index">The ordinal position (zero-based) of the
		/// <see c_ref="CurveItem"/> object to be accessed.</param>
		/// <value>A <see c_ref="CurveItem"/> object reference.</value>
...
*/
		/// <summary>
		/// Indexer to access the specified <see c_ref="CurveItem"/> object by
		/// its <see c_ref="CurveItem.Label"/> string.
		/// </summary>
...
/*
		/// <summary>
		/// Add a <see c_ref="CurveItem"/> object to the collection at the end of the list.
		/// </summary>
		/// <param name="curve">A reference to the <see c_ref="CurveItem"/> object to
		/// be added</param>
...
/*
		/// <summary>
		/// Remove a <see c_ref="CurveItem"/> object from the collection based on an object reference.
		/// </summary>
		/// <param name="curve">A reference to the <see c_ref="CurveItem"/> object that is to be
		/// removed.</param>
...
/*
		/// <summary>
		/// Insert a <see c_ref="CurveItem"/> object into the collection at the specified
...
*/
		...
		/// <see c_ref="CurveItem"/> with the specified <see c_ref="CurveItem.Label"/>.
		/// </summary>
		/// <param name="label">The <see c_ref="String"/> label that is in the
		/// <see c_ref="CurveItem.Label"/> attribute of the item to be found.
		/// </param>
		/// <returns>The zero-based index of the specified <see c_ref="CurveItem"/>,
		/// or -1 if the <see c_ref="CurveItem"/> is not in the list</returns>
		...
		/// <summary>
		/// Return the zero-based position index of the
		/// <see c_ref="CurveItem"/> with the specified <see c_ref="CurveItem.Tag"/>.
		/// </summary>
		/// <remarks>In order for this method to work, the <see c_ref="CurveItem.Tag"/>
		/// property must be of type <see c_ref="String"/>.</remarks>
		/// <param name="tag">The <see c_ref="String"/> tag that is in the
		/// <see c_ref="CurveItem.Tag"/> attribute of the item to be found.
		/// </param>
		/// <returns>The zero-based index of the specified <see c_ref="CurveItem"/>,
		/// or -1 if the <see c_ref="CurveItem"/> is not in the list</returns>
	...
	#region Rendering Methods
		...
		/// <summary>
		/// Go through each <see c_ref="CurveItem"/> object in the collection,
		/// calling the <see c_ref="CurveItem.GetRange"/> member to 
		/// determine the minimum and maximum values in the
		/// <see c_ref="CurveItem.Points"/> list of data value pairs.  If the curves include 
		...
		/// If any <see c_ref="CurveItem"/> in the list has a missing
		/// <see c_ref="PointPairList"/>, a new empty one will be generated.
		/// </summary>
		...
		/// <summary>
		/// Find the ordinal position of the specified <see c_ref="BarItem" /> within
		/// the <see c_ref="CurveList" />.  This position only counts <see cef="BarItem" />
		/// types, ignoring all other types.
		/// </summary>
		/// <param name="pane">The <see c_ref="GraphPane" /> of interest</param>
		/// <param name="barItem">The <see c_ref="BarItem" /> for which to search.</param>
