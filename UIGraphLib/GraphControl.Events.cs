...
namespace UIGraphLib
{
	...
	#region Events
		...
		/// </summary>
		/// <param name="sender">The source <see c_ref="ZedGraphControl"/> object</param>
		/// <param name="menuStrip">A reference to the <see c_ref="ContextMenuStrip"/> object
		...
		/// <seealso c_ref="ContextMenuBuilder" />
		public delegate void ContextMenuBuilderEventHandler( GraphControl sender,
		...
		/// </summary>
		/// <param name="sender">The source <see c_ref="ZedGraphControl"/> object</param>
		/// <param name="oldState">A <see c_ref="ZoomState"/> object that corresponds to the state of the
		/// <see c_ref="GraphPane"/> before the zoom or pan event.</param>
		/// <param name="newState">A <see c_ref="ZoomState"/> object that corresponds to the state of the
		/// <see c_ref="GraphPane"/> after the zoom or pan event</param>
		/// <seealso c_ref="ZoomEvent" />
		public delegate void ZoomEventHandler( GraphControl sender, ZoomState oldState,
			ZoomState newState );

		/// <summary>
		/// Subscribe to this event to be notified when the <see c_ref="GraphPane"/> is zoomed or panned by the user,
		...
		/// </summary>
		/// <param name="sender">The source <see c_ref="ZedGraphControl"/> object</param>
		/// <param name="scrollBar">The source <see c_ref="ScrollBar"/> object</param>
		/// <param name="oldState">A <see c_ref="ZoomState"/> object that corresponds to the state of the
		/// <see c_ref="GraphPane"/> before the scroll event.</param>
		/// <param name="newState">A <see c_ref="ZoomState"/> object that corresponds to the state of the
		/// <see c_ref="GraphPane"/> after the scroll event</param>
		/// <seealso c_ref="ZoomEvent" />
		public delegate void ScrollDoneHandler( GraphControl sender, ScrollBar scrollBar,
			ZoomState oldState, ZoomState newState );

		/// <summary>
		/// Subscribe to this event to be notified when the <see c_ref="GraphPane"/> is scrolled by the user
		...
		/// <see c_ref="GraphPane"/> after the scroll event</param>
		/// <seealso c_ref="ZoomEvent" />
		public delegate void ScrollProgressHandler( GraphControl sender, ScrollBar scrollBar,
			ZoomState oldState, ZoomState newState );
		...
		/// </summary>
		/// <param name="sender">The source <see c_ref="ZedGraphControl"/> object</param>
		/// <param name="pane">The <see c_ref="GraphPane"/> object that contains the
		/// point that has been edited</param>
		/// <param name="curve">The <see c_ref="CurveItem"/> object that contains the point
		/// that has been edited</param>
		/// <param name="iPt">The integer index of the edited <see c_ref="PointPair"/> within the
		/// <see c_ref="IPointList"/> of the selected <see c_ref="CurveItem"/>
		/// </param>
		/// <seealso c_ref="PointValueEvent" />
		public delegate string PointEditHandler( GraphControl sender, GraphPane pane,
			CurveItem curve, int iPt );

		/// <summary>
		/// Subscribe to this event to receive notifcation and/or respond after a data
		/// point has been edited via <see c_ref="IsEnableHEdit" /> and <see c_ref="IsEnableVEdit" />.
		/// </summary>
		/// <example>
		...
		/// </summary>
		/// <param name="sender">The source <see c_ref="ZedGraphControl"/> object</param>
		/// <param name="pane">The <see c_ref="GraphPane"/> object that contains the point value of interest</param>
		/// <param name="curve">The <see c_ref="CurveItem"/> object that contains the point value of interest</param>
		/// <param name="iPt">The integer index of the selected <see c_ref="PointPair"/> within the
		/// <see c_ref="IPointList"/> of the selected <see c_ref="CurveItem"/></param>
		/// <seealso c_ref="PointValueEvent" />
		public delegate string PointValueHandler( GraphControl sender, GraphPane pane,
			CurveItem curve, int iPt );
		...
		/// </summary>
		/// <param name="sender">The source <see c_ref="ZedGraphControl"/> object</param>
		/// <param name="pane">The <see c_ref="GraphPane"/> object that contains the cursor of interest</param>
        /// <param name="mousePt">The <see c_ref="Point"/> object that represents the cursor value location</param>
		/// <seealso c_ref="CursorValueEvent" />
		public delegate string CursorValueHandler( GraphControl sender, GraphPane pane,
			Point mousePt );
		...
		/// </summary>
		/// <param name="sender">The source <see c_ref="ZedGraphControl"/> object</param>
		/// <param name="e">A <see c_ref="MouseEventArgs" /> corresponding to this event</param>
		/// <seealso c_ref="MouseDownEvent" />
		/// <returns>
		/// Return true if you have handled the mouse event entirely, and you do not
		/// want the <see c_ref="ZedGraphControl"/> to do any further action (e.g., starting
		/// a zoom operation).  Return false if ZedGraph should go ahead and process the
		/// mouse event.
		/// </returns>
		public delegate bool ZedMouseEventHandler( GraphControl sender, MouseEventArgs e );
		...
		/// within any <see c_ref="GraphPane"/> of the <see c_ref="MasterPane"/> associated
		/// with this <see c_ref="ZedGraphControl" />.  This event will use the
		/// <see c_ref="ZedGraph.MasterPane.FindNearestPaneObject"/> method to determine which object
		/// was clicked.  The boolean value that you return from this handler determines whether
		/// or not the <see c_ref="ZedGraphControl"/> will do any further handling of the
		/// MouseDown event (see <see c_ref="ZedMouseEventHandler" />).  Return true if you have
		...
		/// was clicked.  The boolean value that you return from this handler determines whether
		/// or not the <see c_ref="ZedGraphControl"/> will do any further handling of the
		/// MouseUp event (see <see c_ref="ZedMouseEventHandler" />).  Return true if you have
		...
		/// or not the <see c_ref="ZedGraphControl"/> will do any further handling of the
		/// MouseMove event (see <see c_ref="ZedMouseEventHandler" />).  Return true if you
		/// have handled the MouseMove event entirely, and you do not
		/// want the <see c_ref="ZedGraphControl"/> to do any further action.
		...
		/// was clicked.  The boolean value that you return from this handler determines whether
		/// or not the <see c_ref="ZedGraphControl"/> will do any further handling of the
		/// DoubleClick event (see <see c_ref="ZedMouseEventHandler" />).  Return true if you have
		...
		/// </summary>
		/// <param name="sender">The source <see c_ref="ZedGraphControl"/> object</param>
		/// <param name="pane">The source <see c_ref="GraphPane" /> in which the click
		/// occurred.
		/// </param>
		/// <param name="source">The source object which was clicked.  This is typically
		/// a type of <see c_ref="CurveItem" /> if a curve point was clicked, or
		/// a type of <see c_ref="GraphObj" /> if a graph object was clicked.
		/// </param>
		/// <param name="link">The <see c_ref="Link" /> object, belonging to
		/// <paramref name="source" />, that contains the link information
		/// </param>
		/// <param name="index">An index value, typically used if a <see c_ref="CurveItem" />
		...
		/// </returns>
		public delegate bool LinkEventHandler( GraphControl sender, GraphPane pane,
		...
		/// </summary>
		/// <remarks>
		/// Linked objects are typically either <see c_ref="GraphObj" /> type objects or
		/// <see c_ref="CurveItem" /> type objects.  These object types can include
		/// hyperlink information allowing for "drill-down" type operation.  
		/// </remarks>
		/// <seealso c_ref="LinkEventHandler"/>
		/// <seealso c_ref="Link" />
		/// <seealso c_ref="CurveItem.Link">CurveItem.Link</seealso>
		/// <seealso c_ref="GraphObj.Link">GraphObj.Link</seealso>
	...
	#region Mouse Events
		...
		/// <summary>
		/// Handle a MouseDown event in the <see c_ref="ZedGraphControl" />
		/// </summary>
		/// <param name="sender">A reference to the <see c_ref="ZedGraphControl" /></param>
		/// <param name="e">A <see c_ref="MouseEventArgs" /> instance</param>
		...
		/// </summary>
		/// <param name="sender">The <see c_ref="ZedGraphControl" /> in which the KeyUp occurred.</param>
		/// <param name="e">A <see c_ref="KeyEventArgs" /> instance.</param>
		...
		/// <summary>
		/// Handle a MouseUp event in the <see c_ref="ZedGraphControl" />
		/// </summary>
		/// <param name="sender">A reference to the <see c_ref="ZedGraphControl" /></param>
		/// <param name="e">A <see c_ref="MouseEventArgs" /> instance</param>
	...
	#region Mouse Wheel Zoom Events
		...
		/// <summary>
		/// Handle a MouseWheel event in the <see c_ref="ZedGraphControl" />
		/// </summary>
		/// <param name="sender">A reference to the <see c_ref="ZedGraphControl" /></param>
		/// <param name="e">A <see c_ref="MouseEventArgs" /> instance</param>
		...
		/// <param name="axis">The <see c_ref="Axis" /> to be zoomed.</param>
	...
	#region Pan Events
		...
		/// <summary>
		/// Handle a panning operation for the specified <see c_ref="Axis" />.
		/// </summary>
		/// <param name="axis">The <see c_ref="Axis" /> to be panned</param>
