...
{
	/// <summary>
	/// A class that encapsulates color-fill properties for an object.  The <see c_ref="Fill"/> class
	/// is used in <see c_ref="PaneBase.Rect"/>, <see c_ref="Chart.Rect"/>, <see c_ref="Legend"/>,
	/// <see c_ref="Bar"/>, and <see c_ref="Line"/> objects.
	/// </summary>
	/// 
	...
	#region Fields
		...
		/// property <see c_ref="Color"/> to access this value.  This property is
		/// only applicable if the <see c_ref="Type"/> is not <see c_ref="ZedGraph.FillType.None"/>.
		/// </summary>
		private Color _color;
		/// <summary>
		/// Private field that stores the secondary color for gradientByValue fills.  Use the public
		/// property <see c_ref="SecondaryValueGradientColor"/> to access this value.  This property is
		/// only applicable if the <see c_ref="Type"/> is <see c_ref="ZedGraph.FillType.GradientByX"/>,
		/// <see c_ref="ZedGraph.FillType.GradientByY"/>, or <see c_ref="ZedGraph.FillType.GradientByZ"/>.
		/// </summary>
		private Color _secondaryValueGradientColor;
		/// <summary>
		/// Private field that stores the custom fill brush.  Use the public
		/// property <see c_ref="Brush"/> to access this value.  This property is
		/// only applicable if the 
		/// <see c_ref="Type"/> property is set to <see c_ref="ZedGraph.FillType.Brush"/>.
		/// </summary>
		protected Brush	_brush;
		/// <summary>
		/// Private field that determines the type of color fill.  Use the public
		/// property <see c_ref="Type"/> to access this value.  The fill color
		/// is determined by the property <see c_ref="Color"/> or
		/// <see c_ref="Brush"/>.
		/// </summary>
		...
		/// with the filled object based on the <see c_ref="AlignH"/> and <see c_ref="AlignV"/>
		/// properties.
		/// </summary>
		private bool		_isScaled;
		/// <summary>
		/// Private field that determines how the brush will be aligned with the filled object
		/// in the horizontal direction.  This value is a <see c_ref="ZedGraph.AlignH"/> enumeration.
		/// This field only applies if <see c_ref="IsScaled"/> is false.
		/// properties.
		/// </summary>
		/// <seealso c_ref="AlignH"/>
		/// <seealso c_ref="AlignV"/>
		private AlignH		_alignH;
		/// <summary>
		/// Private field that determines how the brush will be aligned with the filled object
		/// in the vertical direction.  This value is a <see c_ref="ZedGraph.AlignV"/> enumeration.
		/// This field only applies if <see c_ref="IsScaled"/> is false.
		/// properties.
		/// </summary>
		/// <seealso c_ref="AlignH"/>
		...
		/// <see c_ref="LinearGradientBrush"/> in the constructor.  This is used strictly
		/// for serialization.
		/// </summary>
	...
	#region Defaults
		...
		/// default property values for the <see c_ref="Fill"/> class.
		/// </summary>
		public struct Default
		{
			// Default Fill properties
			/// <summary>
			/// The default scaling mode for <see c_ref="Brush"/> fills.
			/// This is the default value for the <see c_ref="Fill.IsScaled"/> property.
			/// </summary>
			public static bool IsScaled = true;
			/// <summary>
			/// The default horizontal alignment for <see c_ref="Brush"/> fills.
			/// This is the default value for the <see c_ref="Fill.AlignH"/> property.
			/// </summary>
			public static AlignH AlignH = AlignH.Center;
			/// <summary>
			/// The default vertical alignment for <see c_ref="Brush"/> fills.
			/// This is the default value for the <see c_ref="Fill.AlignV"/> property.
			/// </summary>
	...
	#region Constructors
		...
		/// <param name="brush">A custom brush for fills.  Can be a <see c_ref="SolidBrush"/>,
		/// <see c_ref="LinearGradientBrush"/>, or <see c_ref="TextureBrush"/>.</param>
		/// <param name="type">The <see c_ref="FillType"/> for this fill.</param>
		...
		/// <summary>
		/// Constructor that creates a solid color-fill, setting <see c_ref="Type"/> to
		/// <see c_ref="FillType.Solid"/>, and setting <see c_ref="Color"/> to the
		...
		/// <summary>
		/// Constructor that creates a linear gradient color-fill, setting <see c_ref="Type"/> to
		/// <see c_ref="FillType.Brush"/> using the specified colors and angle.
		/// </summary>
		...
		/// <summary>
		/// Constructor that creates a linear gradient color-fill, setting <see c_ref="Type"/> to
		/// <see c_ref="FillType.Brush"/> using the specified colors.
		/// </summary>
		/// <param name="color1">The first color for the gradient fill</param>
		/// <param name="color2">The second color for the gradient fill</param>
		...
		/// <summary>
		/// Constructor that creates a linear gradient color-fill, setting <see c_ref="Type"/> to
		/// <see c_ref="FillType.Brush"/> using the specified colors.  This gradient fill
		...
		/// <summary>
		/// Constructor that creates a linear gradient multi-color-fill, setting <see c_ref="Type"/> to
		/// <see c_ref="FillType.Brush"/> using the specified colors.  This gradient fill
		/// consists of many colors based on a <see c_ref="ColorBlend"/> object.  The gradient
		/// angle is defaulted to zero.
		/// </summary>
		/// <param name="blend">The <see c_ref="ColorBlend"/> object that defines the colors
		...
		/// Constructor that creates a linear gradient multi-color-fill, setting <see c_ref="Type"/> to
		/// <see c_ref="FillType.Brush"/> using the specified colors.  This gradient fill
		/// consists of many colors based on a <see c_ref="ColorBlend"/> object, drawn at the
		...
		/// Constructor that creates a linear gradient multi-color-fill, setting <see c_ref="Type"/> to
		/// <see c_ref="FillType.Brush"/> using the specified colors.  This gradient fill
		/// consists of many colors based on an array of <see c_ref="Color"/> objects, drawn at an
		/// angle of zero (degrees).  The <see paramref="colors"/> array is used to create
		/// a <see c_ref="ColorBlend"/> object assuming a even linear distribution of the colors
		/// across the gradient.
		/// </summary>
		/// <param name="colors">The array of <see c_ref="Color"/> objects that defines the colors
		...
		/// Constructor that creates a linear gradient multi-color-fill, setting <see c_ref="Type"/> to
		/// <see c_ref="FillType.Brush"/> using the specified colors.  This gradient fill
		/// consists of many colors based on an array of <see c_ref="Color"/> objects, drawn at the
		...
		/// <summary>
		/// Constructor that creates a texture fill, setting <see c_ref="Type"/> to
		/// <see c_ref="FillType.Brush"/> and using the specified image.
		/// </summary>
		/// <param name="image">The <see c_ref="Image"/> to use for filling</param>
		/// <param name="wrapMode">The <see c_ref="WrapMode"/> class that controls the image wrapping properties</param>
		...
		/// <summary>
		/// Constructor that creates a <see c_ref="Brush"/> fill, using a user-supplied, custom
		/// <see c_ref="Brush"/>.  The brush will be scaled to fit the destination screen object
		/// unless you manually change <see c_ref="IsScaled"/> to false;
		/// </summary>
		/// <param name="brush">The <see c_ref="Brush"/> to use for fancy fills.  Typically, this would
		/// be a <see c_ref="LinearGradientBrush"/> or a <see c_ref="TextureBrush"/> class</param>
		...
		/// <summary>
		/// Constructor that creates a <see c_ref="Brush"/> fill, using a user-supplied, custom
		/// <see c_ref="Brush"/>.  This constructor will make the brush unscaled (see <see c_ref="IsScaled"/>),
	...
	#region Properties
		...
		/// (<see c_ref="Type"/> is <see c_ref="FillType.Solid"/>), or it can be used in 
		/// combination with <see c_ref="System.Drawing.Color.White"/> to make a
		/// <see c_ref="LinearGradientBrush"/>
		/// when <see c_ref="Type"/> is <see c_ref="FillType.Brush"/> and <see c_ref="Brush"/>
		/// is null.
		/// </summary>
		/// <seealso c_ref="Type"/>
		public Color Color
		...
		/// </summary>
		/// <remarks>
		/// This property is only applicable if the <see c_ref="Type"/> is
		/// <see c_ref="ZedGraph.FillType.GradientByX"/>,
		/// <see c_ref="ZedGraph.FillType.GradientByY"/>, or
		/// <see c_ref="ZedGraph.FillType.GradientByZ"/>.  Once the gradient-by-value logic picks
		...
		/// <see c_ref="Fill" />. Use a value of <see c_ref="System.Drawing.Color.Empty">Color.Empty</see> to have
		/// a solid-color <see c_ref="Fill" /> resulting from a gradient-by-value
		/// <see c_ref="FillType" />.
		/// </remarks>
		...
		/// <summary>
		/// The custom fill brush.  This can be a <see c_ref="SolidBrush"/>, a
		/// <see c_ref="LinearGradientBrush"/>, or a <see c_ref="TextureBrush"/>.  This property is
		/// only applicable if the <see c_ref="Type"/> property is set
		/// to <see c_ref="FillType.Brush"/>.
		/// </summary>
		public Brush Brush
		{
			get { return _brush; }
		...
		/// color (<see c_ref="ZedGraph.FillType.Solid"/>) or a custom brush
		/// (<see c_ref="ZedGraph.FillType.Brush"/>).  See <see c_ref="Type"/> for
		/// more information.
		/// </summary>
		/// <seealso c_ref="ZedGraph.Fill.Color"/>
		...
		/// Returns true if the <see c_ref="Type"/> property is either
		/// <see c_ref="FillType.Solid"/> or
		/// <see c_ref="FillType.Brush"/>.  If set to true, this property
		/// will automatically set the <see c_ref="Type"/> to
		/// <see c_ref="FillType.Brush"/>.  If set to false, this property
		/// will automatically set the <see c_ref="Type"/> to
		/// <see c_ref="FillType.None"/>.  In order to get a regular
		/// solid-color fill, you have to manually set <see c_ref="Type"/>
		/// to <see c_ref="FillType.Solid"/>.
		/// </summary>
		/// <seealso c_ref="Color"/>
		/// <seealso c_ref="Brush"/>
		/// <seealso c_ref="Type"/>
		...
		/// type.  This is true for <see c_ref="FillType.GradientByX"/>, <see c_ref="FillType.GradientByY"/>,
		/// or <see c_ref="FillType.GradientByZ"/>.
		/// </summary>
		/// <remarks>
		/// The gradient by value fill method allows the fill color for each point or bar to
		/// be based on a value for that point (either X, Y, or Z in the <see c_ref="IPointList"/>.
		/// For example, assume a <see c_ref="Fill"/> class is defined with a linear gradient ranging from
		/// <see c_ref="System.Drawing.Color.Blue"/> to <see c_ref="System.Drawing.Color.Red"/> and the <see c_ref="Fill.Type"/>
		/// is set to <see c_ref="FillType.GradientByY"/>.  If <see c_ref="RangeMin"/> is set to 
		/// 100.0 and <see c_ref="RangeMax"/> is set to 200.0, then a point that has a Y value of
		...
		/// You can use the Z value from <see c_ref="IPointList"/> along with
		/// <see c_ref="FillType.GradientByZ"/> to color individual points according to some
		/// property that is independent of the X,Y point pair.
		/// </remarks>
		/// <value>true if this is a Gradient-by-value type, false otherwise</value>
		/// <seealso c_ref="FillType.GradientByX"/>
		/// <seealso c_ref="FillType.GradientByY"/>
		/// <seealso c_ref="FillType.GradientByZ"/>
		...
		/// <seealso c_ref="FillType.GradientByY"/>
		/// <seealso c_ref="FillType.GradientByZ"/>
		/// <seealso c_ref="IsGradientValueType"/>
		/// <seealso c_ref="RangeMax"/>
		/// <seealso c_ref="RangeDefault"/>
