...
{	
	...
	/// </summary>
	/// <seealso c_ref="ZedGraph.Axis.Type"/>
	public enum AxisType
	{
		...
		/// for points corresponding to that axis are ignored.  That is, if the X axis is an
		/// ordinal type, then all X values associated with the curves are ignored. </summary>
		/// <seealso c_ref="AxisType.Text"/>
		/// <seealso c_ref="Scale.IsOrdinal"/>
		Ordinal,
		/// <summary> An ordinal axis that will have labels formatted with ordinal values corresponding
		/// to the number of values in each <see c_ref="CurveItem" />.
		/// </summary>
		/// <remarks>
		/// The <see c_ref="CurveItem" /> data points will be evenly-spaced at ordinal locations, and the
	...
	/// <summary>
	/// Enumeration type for the various types of fills that can be used with <see c_ref="Bar"/>
	/// charts.
	/// </summary>
	public enum FillType
	{
		...
		None,
		/// <summary> A solid fill using <see c_ref="System.Drawing.SolidBrush"/> </summary>
		Solid,
		/// <summary> A custom fill using either <see c_ref="LinearGradientBrush"/> or
		/// <see c_ref="TextureBrush"/></summary>
		Brush,
		/// <summary>
		...
		/// of <see c_ref="Fill.RangeMin"/> and <see c_ref="Fill.RangeMax"/>.  You can create a multicolor
		/// range by initializing the <see c_ref="Fill"/> class with your own custom
		/// <see c_ref="Brush"/> object based on a <see c_ref="ColorBlend"/>.  In cases where a
		/// data value makes no sense (<see c_ref="PaneBase.Fill"/>, <see c_ref="Legend.Fill"/>,
		/// etc.), a default value of 50% of the range is assumed.  The default range is 0 to 1.
		/// </remarks>
		/// <seealso c_ref="Fill.RangeMin"/>
		/// <seealso c_ref="Fill.RangeMax"/>
		/// <seealso c_ref="Fill.RangeDefault"/>
		GradientByX,
		/// <summary>
	...
	{
		/// <summary> Yearly units <see c_ref="Scale.MajorUnit"/> and <see c_ref="Scale.MinorUnit"/>
		/// </summary>
		Year,
		/// <summary> Monthly units <see c_ref="Scale.MajorUnit"/> and <see c_ref="Scale.MinorUnit"/>
		/// </summary>
		Month,
		/// <summary> Daily units <see c_ref="Scale.MajorUnit"/> and <see c_ref="Scale.MinorUnit"/>
		/// </summary>
		Day,
		/// <summary> Hourly units <see c_ref="Scale.MajorUnit"/> and <see c_ref="Scale.MinorUnit"/>
		/// </summary>
		Hour,
		/// <summary> Minute units <see c_ref="Scale.MajorUnit"/> and <see c_ref="Scale.MinorUnit"/>
		/// </summary>
		Minute,
		/// <summary> Second units <see c_ref="Scale.MajorUnit"/> and <see c_ref="Scale.MinorUnit"/>
		/// </summary>
		Second,
		/// <summary> Millisecond units <see c_ref="Scale.MajorUnit"/> and <see c_ref="Scale.MinorUnit"/>
		/// </summary>
		Millisecond
	}
	...
	/// </summary>
	/// <seealso c_ref="ZedGraph.Symbol.Fill"/>
	public enum SymbolType 
	{
		/// <summary> Square-shaped <see c_ref="ZedGraph.Symbol"/> </summary>
		Square,
		/// <summary> Rhombus-shaped <see c_ref="ZedGraph.Symbol"/> </summary>
		Diamond,
		/// <summary> Equilateral triangle <see c_ref="ZedGraph.Symbol"/> </summary>
		Triangle,
		/// <summary> Uniform circle <see c_ref="ZedGraph.Symbol"/> </summary>
		Circle,
		/// <summary> "X" shaped <see c_ref="ZedGraph.Symbol"/>.  This symbol cannot
		/// be filled since it has no outline. </summary>
		XCross,
		/// <summary> "+" shaped <see c_ref="ZedGraph.Symbol"/>.  This symbol cannot
		/// be filled since it has no outline. </summary>
		Plus,
		/// <summary> Asterisk-shaped <see c_ref="ZedGraph.Symbol"/>.  This symbol
		/// cannot be filled since it has no outline. </summary>
		Star,
		/// <summary> Unilateral triangle <see c_ref="ZedGraph.Symbol"/>, pointing
		/// down. </summary>
		TriangleDown,
		/// <summary>
		/// Horizontal dash <see c_ref="ZedGraph.Symbol"/>.  This symbol cannot be
		/// filled since it has no outline.
		/// </summary>
		HDash,
		/// <summary>
		/// Vertical dash <see c_ref="ZedGraph.Symbol"/>.  This symbol cannot be
		/// filled since it has no outline.
		/// </summary>
		VDash,
		/// <summary> A symbol defined by the <see c_ref="Symbol.UserSymbol"/> propery.
		/// If no symbol is defined, the <see c_ref="Symbol.Default.Type"/>. symbol will
		/// be used.
		/// </summary>
		UserDefined,
		/// <summary> A Default symbol type (the symbol type will be obtained
		/// from <see c_ref="Symbol.Default.Type"/>. </summary>
		Default,
		/// <summary> No symbol is shown (this is equivalent to using
		/// <see c_ref="Symbol.IsVisible"/> = false.</summary>
		None
	}
	...
	/// </summary>
	/// <seealso c_ref="Legend.Position"/>
	public enum LegendPos
	{
		/// <summary>
		/// Locate the <see c_ref="Legend"/> above the <see c_ref="Chart.Rect"/>
		/// </summary>
		Top,
		/// <summary>
		/// Locate the <see c_ref="Legend"/> on the left side of the <see c_ref="Chart.Rect"/>
		/// </summary>
		Left,
		/// <summary>
		/// Locate the <see c_ref="Legend"/> on the right side of the <see c_ref="Chart.Rect"/>
		/// </summary>
		Right,
		/// <summary>
		/// Locate the <see c_ref="Legend"/> below the <see c_ref="Chart.Rect"/>
		/// </summary>
		Bottom,
		/// <summary>
		/// Locate the <see c_ref="Legend"/> inside the <see c_ref="Chart.Rect"/> in the
		/// top-left corner.  
		/// </summary>
		InsideTopLeft,
		/// <summary>
		...
		InsideBotRight,
		/// <summary>
		/// Locate the <see c_ref="Legend"/> as a floating object above the graph at the
		/// location specified by <see c_ref="Legend.Location"/>.
		/// </summary>
		Float,
		/// <summary>
		/// Locate the <see c_ref="Legend"/> centered above the <see c_ref="Chart.Rect"/>
		/// </summary>
		TopCenter,
		/// <summary>
		/// Locate the <see c_ref="Legend"/> centered below the <see c_ref="Chart.Rect"/>
		/// </summary>
		BottomCenter,
		/// <summary>
		/// Locate the <see c_ref="Legend"/> above the <see c_ref="Chart.Rect"/>, but flush
		/// against the left margin of the <see c_ref="PaneBase.Rect" />.
		/// </summary>
		TopFlushLeft,
		/// <summary>
		/// Locate the <see c_ref="Legend"/> below the <see c_ref="Chart.Rect"/>, but flush
		/// against the left margin of the <see c_ref="PaneBase.Rect" />.
		/// </summary>
		BottomFlushLeft
	...
	public enum AlignH
	{
		...
		/// <see c_ref="FontSpec.Draw(Graphics,PaneBase,string,float,float,AlignH,AlignV,float)"/> method.
		/// </summary>
		Left,
		/// <summary>
	...
	/// These coordinate types are used the <see c_ref="ArrowObj"/> objects
	/// and <see c_ref="TextObj"/> objects only.
	/// </summary>
	/// <seealso c_ref="ZedGraph.Location.CoordinateFrame"/>
	public enum CoordType
	{
		/// <summary>
		/// Coordinates are specified as a fraction of the
		/// <see c_ref="Chart.Rect"/>.  That is, for the X coordinate, 0.0
		...
		/// <summary>
		/// Coordinates are specified as a fraction of the
		/// <see c_ref="PaneBase.Rect"/>.  That is, for the X coordinate, 0.0
		...
		/// for the <see c_ref="GraphPane.XAxis"/> and <see c_ref="GraphPane.YAxis"/>.
		/// </summary>
		AxisXYScale,
		/// <summary>
		/// Coordinates are specified according to the user axis scales
		/// for the <see c_ref="GraphPane.XAxis"/> and <see c_ref="GraphPane.Y2Axis"/>.
		/// </summary>
		AxisXY2Scale,
		/// <summary>
		/// The X coordinate is specified as a fraction of the <see c_ref="Chart.Rect"/>,
		/// and the Y coordinate is specified as a fraction of the <see c_ref="PaneBase.Rect" />.
		/// </summary>
		/// <remarks>
		...
		/// <summary>
		/// The X coordinate is specified as a fraction of the <see c_ref="PaneBase.Rect"/>,
		/// and the Y coordinate is specified as a fraction of the <see c_ref="Chart.Rect" />.
		/// </summary>
		/// <remarks>
		...
		/// <see c_ref="Chart.Rect"/> and the Y coordinate is specified as
		/// a Y scale value.
		/// </summary>
		/// <remarks>
		/// For the X coordinate, 0.0
		...
		/// outside the <see c_ref="Chart.Rect" />.
		/// </remarks>
	...
	/// </summary>
	/// <seealso c_ref="Line.StepType"/>
	public enum StepType
	{
		/// <summary>
		/// Draw the <see c_ref="CurveItem"/> as a stair-step in which each
		/// point defines the
		...
		RearwardStep,
		/// <summary>
		/// Draw the <see c_ref="CurveItem"/> as an ordinary line, in which the
		/// points are connected directly by line segments.
		/// </summary>
		NonStep,
		/// <summary>
		/// Draw the <see c_ref="CurveItem"/> as a segment in which each point defines the
	...
	/// <summary>
	/// Enumeration type that defines the base axis from which <see c_ref="Bar"/> graphs
	/// are displayed. The bars can be drawn on any of the four axes (<see c_ref="XAxis"/>,
	/// <see c_ref="X2Axis"/>, <see c_ref="YAxis"/>, and <see c_ref="Y2Axis"/>).
	/// </summary>
	/// <seealso c_ref="BarSettings.Base"/>
	public enum BarBase
	{
		/// <summary>
		/// Draw the <see c_ref="Bar"/> chart based from the <see c_ref="XAxis"/>.
		/// </summary>
		X,
		/// <summary>
		/// Draw the <see c_ref="Bar"/> chart based from the <see c_ref="X2Axis"/>.
		/// </summary>
		X2,
		/// <summary>
		/// Draw the <see c_ref="Bar"/> chart based from the <see c_ref="YAxis"/>.
		/// </summary>
		Y,
		/// <summary>
		/// Draw the <see c_ref="Bar"/> chart based from the <see c_ref="Y2Axis"/>.
		/// </summary>
		Y2
	}
	
	/// <summary>
	/// Enumeration type that defines the available types of <see c_ref="LineItem"/> graphs.
	/// </summary>
	...
	/// <summary>
	/// Enumeration type that defines the available types of <see c_ref="BarItem"/> graphs.
	/// </summary>
	/// <seealso c_ref="BarSettings.Type"/>
	public enum BarType
	{
		/// <summary>
		/// Draw each <see c_ref="BarItem"/> side by side in clusters.
		/// </summary>
		Cluster,
		/// <summary>
		/// Draw the <see c_ref="BarItem"/> bars one on top of the other.  The bars will
		/// be drawn such that the last bar in the <see c_ref="CurveList"/> will be behind
		...
		/// to <see c_ref="GraphPane.AddBar(string,IPointList,Color)"/>.
		/// For example, if the first bar of
		/// the first <see c_ref="BarItem"/> has a value of 100, and the first bar of
		/// the second <see c_ref="BarItem"/> has a value of 120, then that bar will
		...
		/// This is similar to the <see c_ref="Overlay"/> mode, but the bars are sorted at
		/// each base value.
		/// The data values must be summed before being passed
		/// to <see c_ref="GraphPane.AddBar(string,IPointList,Color)"/>.  For example, if the first bar of
		/// the first <see c_ref="BarItem"/> has a value of 100, and the first bar of
		/// the second <see c_ref="BarItem"/> has a value of 120, then that bar will
		/// appear to be 20 units on top of the first bar.
		/// </summary>
		SortedOverlay,
		/// <summary>
		/// Draw the <see c_ref="BarItem"/> bars in an additive format so that they stack on
		/// top of one another.  The value of the last bar drawn will be the sum of the values
		/// of all prior bars.
		/// </summary>
		Stack,
		/// <summary>
		/// Draw the <see c_ref="BarItem"/> bars in a format whereby the height of each
	...
	/// <seealso c_ref="System.Collections.ArrayList.Sort()"/> to perform the sort.
	/// </summary>
	...
	/// <summary>
	/// Enumeration that specifies a Z-Order position for <see c_ref="GraphObj"/>
	/// objects.
	/// </summary>
	/// <remarks>This enumeration allows you to set the layering of various graph
	/// features.  Except for the <see c_ref="GraphObj"/> objects, other feature types
	/// all have a fixed depth as follows (front to back):
	/// <list>
	/// <see c_ref="Legend"/> objects
	/// The border around <see c_ref="Chart.Rect"/>
	/// <see c_ref="CurveItem"/> objects
	/// The <see c_ref="Axis"/> features
	/// The background fill of the <see c_ref="Chart.Rect"/>
	/// The pane <see c_ref="PaneBase.Title"/>
	/// The background fill of the <see c_ref="PaneBase.Rect"/>
	/// </list>
	/// You cannot place anything behind the <see c_ref="PaneBase.Rect"/>
	/// background fill, but <see c_ref="GraphObj.ZOrder"/> allows you to
	/// explicitly control the depth of <see c_ref="GraphObj"/> objects
	/// between all other object types.  For items of equal <see c_ref="ZOrder"/>,
	/// such as multiple <see c_ref="CurveItem"/>'s or <see c_ref="GraphObj"/>'s
	/// having the same <see c_ref="ZOrder"/> value, the relative depth is
	/// controlled by the ordinal position in the list (either
	/// <see c_ref="CurveList"/> or <see c_ref="GraphObjList"/>).
	/// <see c_ref="GraphObj"/> objects
	/// can be placed in the <see c_ref="GraphObjList"/> of either a
	/// <see c_ref="GraphPane"/> or a <see c_ref="MasterPane"/>.  For a
	/// <see c_ref="GraphPane"/>-based <see c_ref="GraphObj"/>, all <see c_ref="ZOrder"/>
	/// values are applicable.  For a <see c_ref="MasterPane"/>-based
	/// <see c_ref="GraphObj"/>, any <see c_ref="ZOrder"/> value can be used, but there
	/// are really only three depths:
	/// <list><see c_ref="ZOrder.H_BehindAll"/> will place the item behind the pane title,
	/// <see c_ref="ZOrder.A_InFront"/> will place on top of all other graph features,
	/// any other value places the object above the pane title, but behind the <see c_ref="GraphPane"/>'s.
	/// </list>
	/// </remarks>
	public enum ZOrder
	{
	   /// <summary>
	   /// Specifies that the <see c_ref="GraphObj"/> will be behind all other
	   /// objects (including the <see c_ref="PaneBase"/> <see c_ref="PaneBase.Title"/>).
	   /// </summary>
	   H_BehindAll,
	   /// <summary>
	   /// Specifies that the <see c_ref="GraphObj"/> will be behind the
	   /// <see c_ref="Chart.Rect"/> background <see c_ref="Fill"/>
	   /// (see <see c_ref="Chart.Fill"/>).
	   /// </summary>
	   G_BehindChartFill,
		/// <summary>
		/// Specifies that the <see c_ref="GraphObj"/> will be behind the grid lines.
		/// </summary>
		F_BehindGrid,
		/// <summary>
		/// Specifies that the <see c_ref="GraphObj"/> will be behind the
		...
		/// <summary>
	   /// Specifies that the <see c_ref="GraphObj"/> will be behind the
	   /// <see c_ref="Axis"/> objects.
	   /// </summary>
	   D_BehindAxis,
	   /// <summary>
	   /// Specifies that the <see c_ref="GraphObj"/> will be behind the
	   /// <see c_ref="Chart"/> border.
	   /// </summary>
	   C_BehindChartBorder,
	   /// <summary>
	   /// Specifies that the <see c_ref="GraphObj"/> will be behind the
	   /// <see c_ref="Legend"/> object.
	   /// </summary>
	   B_BehindLegend,
	   /// <summary>
	   /// Specifies that the <see c_ref="GraphObj"/> will be in front of
	   /// all other objects, except for the other <see c_ref="GraphObj"/>
	   /// objects that have the same <see c_ref="ZOrder"/> and are before
	   /// this object in the <see c_ref="GraphObjList"/>.
	   /// </summary>
	   A_InFront
	}
	...
	/// (see <see c_ref="PieItem.LabelType"/>).
	/// </summary>
	public enum PieLabelType
	{
		/// <summary>
		/// Displays <see c_ref="CurveItem.Label"/> and <see c_ref="PieItem.Value"/> for
		/// a slice in a Pie Chart.
		/// </summary>
		Name_Value,

		/// <summary>
		/// Displays <see c_ref="CurveItem.Label"/> and <see c_ref="PieItem.Value"/> (as % of total) for
		/// a slice in a Pie Chart.
		/// </summary>
		Name_Percent,		

		/// <summary>
		/// Displays a <see c_ref="CurveItem.Label"/> containing the <see c_ref="PieItem.Value"/> both  
		/// as an absolute number and as percentage of the total.
		/// </summary>
		Name_Value_Percent,		

		/// <summary>
		/// Displays <see c_ref="PieItem.Value"/> for
		/// a slice in a Pie Chart.
		/// </summary>
		Value,

		/// <summary>
		/// Displays <see c_ref="PieItem.Value"/> (as % of total) for
		/// a slice in a Pie Chart.
		/// </summary>
		Percent,

		/// <summary>
		/// Displays <see c_ref="CurveItem.Label"/> for a slice in a Pie Chart.
		/// </summary>
		Name,
	...
	/// <see c_ref="MasterPane.SetLayout(Graphics,PaneLayout)"/> method.
	/// </summary>
	public enum PaneLayout
	{
		/// <summary>
		/// Layout the <see c_ref="GraphPane"/>'s so they are in a square grid (always 2x2, 3x3, 4x4),
		...
		ForceSquare,
		/// <summary>
		/// Layout the <see c_ref="GraphPane"/>'s so they are in a general square (2x2, 3x3, etc.), but use extra
		...
		SquareRowPreferred,
		/// <summary>
		/// Layout the <see c_ref="GraphPane"/>'s in a single row
		/// </summary>
		SingleRow,
		/// <summary>
		/// Layout the <see c_ref="GraphPane"/>'s in a single column
		/// </summary>
		SingleColumn,
		/// <summary>
		/// Layout the <see c_ref="GraphPane"/>'s with an explicit number of columns: The first row has
		...
		ExplicitCol32,
		/// <summary>
		/// Layout the <see c_ref="GraphPane"/>'s with an explicit number of rows: The first column has
