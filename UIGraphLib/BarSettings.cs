...
namespace UIGraphLib
{
	...
	#region Fields
		...
		/// Use the public property <see c_ref="MinClusterGap"/> to access this value. </summary>
		...
		/// Use the public property <see c_ref="MinBarGap"/> to access this value. </summary>
		private float _minBarGap;
		/// <summary>Private field that determines the base axis from which <see c_ref="Bar"/>
		/// graphs will be displayed.  The base axis is the axis from which the bars grow with
		/// increasing value. The value is of the enumeration type <see c_ref="ZedGraph.BarBase"/>.
		/// To access this value, use the public property <see c_ref="Base"/>.
		/// </summary>
		/// <seealso c_ref="Default.Base"/>
		private BarBase _base;
		/// <summary>Private field that determines how the <see c_ref="BarItem"/>
		/// graphs will be displayed. See the <see c_ref="ZedGraph.BarType"/> enum
		/// for the individual types available.
		/// To access this value, use the public property <see c_ref="Type"/>.
		/// </summary>
		/// <seealso c_ref="Default.Type"/>
		...
		/// <see c_ref="AxisType.Ordinal"/> or <see c_ref="AxisType.Text"/>, and the bars are
		/// defined at ordinal values (1.0 scale units apart).  For <see c_ref="AxisType.Linear"/>
		/// or other scale types, you can use this value to scale the bars to an arbitrary
		/// user scale. Use the public property <see c_ref="ClusterScaleWidth"/> to access this
		/// value. </summary>
		internal double _clusterScaleWidth;

		/// <summary>
		/// Private field that determines if the <see c_ref="ClusterScaleWidth" /> will be
		/// calculated automatically.  Use the public property <see c_ref="ClusterScaleWidthAuto" />
	...
	#region Bar Properties
		...
		/// <summary>
		/// The minimum space between <see c_ref="Bar"/> clusters, expressed as a
		/// fraction of the bar size.
		/// </summary>
		/// <seealso c_ref="Default.MinClusterGap"/>
		/// <seealso c_ref="MinBarGap"/>
		/// <seealso c_ref="ClusterScaleWidth"/>
		...
		/// <summary>
		/// The minimum space between individual <see c_ref="Bar">Bars</see>
		/// within a cluster, expressed as a
		/// fraction of the bar size.
		/// </summary>
		/// <seealso c_ref="Default.MinBarGap"/>
		/// <seealso c_ref="MinClusterGap"/>
		/// <seealso c_ref="ClusterScaleWidth"/>
		...
		/// <summary>Determines the base axis from which <see c_ref="Bar"/>
		...
		/// <summary>Determines how the <see c_ref="BarItem"/>
		/// graphs will be displayed. See the <see c_ref="ZedGraph.BarType"/> enum
		/// for the individual types available.
		/// </summary>
		/// <seealso c_ref="Default.Type"/>
		...
		/// <summary>
		/// The width of an individual bar cluster on a <see c_ref="Bar"/> graph.
		/// This value only applies to bar graphs plotted on non-ordinal X axis
		/// types (<see c_ref="AxisType.Linear"/>, <see c_ref="AxisType.Log"/>, and
		/// <see c_ref="AxisType.Date"/>.
		/// </summary>
		/// <remarks>
		/// This value can be calculated automatically if <see c_ref="ClusterScaleWidthAuto" />
		/// is set to true.  In this case, ClusterScaleWidth will be calculated if
		/// <see c_ref="Base" /> refers to an <see c_ref="Axis" /> of a non-ordinal type
		/// (<see c_ref="Scale.IsAnyOrdinal" /> is false).  The ClusterScaleWidth is calculated
		/// from the minimum difference found between any two points on the <see c_ref="Base" />
		/// <see c_ref="Axis" /> for any <see c_ref="BarItem" /> in the
		/// <see c_ref="GraphPane.CurveList" />.  The ClusterScaleWidth is set automatically
		/// each time <see c_ref="GraphPane.AxisChange()" /> is called.  Calculations are
		/// done by the <see c_ref="BarSettings.CalcClusterScaleWidth" /> method.
		/// </remarks>
		/// <seealso c_ref="Default.ClusterScaleWidth"/>
		/// <seealso c_ref="ClusterScaleWidthAuto"/>
		/// <seealso c_ref="MinBarGap"/>
		/// <seealso c_ref="MinClusterGap"/>
		...
		/// <summary>
		/// Gets or sets a property that determines if the <see c_ref="ClusterScaleWidth" /> will be
		/// calculated automatically.
		/// </summary>
		/// <remarks>true for the <see c_ref="ClusterScaleWidth" /> to be calculated
		/// automatically based on the available data, false otherwise.  This value will
		/// be set to false automatically if the <see c_ref="ClusterScaleWidth" /> value
		/// is changed by the user.
		/// </remarks>
		/// <seealso c_ref="Default.ClusterScaleWidthAuto"/>
		/// <seealso c_ref="ClusterScaleWidth"/>
	...
	#region Methods
		...
		/// <summary>
		/// Calculate the width of an individual bar cluster on a <see c_ref="BarItem"/> graph.
		/// This value only applies to bar graphs plotted on non-ordinal X axis
		/// types (<see c_ref="Scale.IsAnyOrdinal" /> is false).
		/// </summary>
		/// <remarks>
		/// This value can be calculated automatically if <see c_ref="ClusterScaleWidthAuto" />
		/// is set to true.  In this case, ClusterScaleWidth will be calculated if
		...
		/// <see c_ref="Axis" /> for any <see c_ref="BarItem" /> in the
		/// <see c_ref="GraphPane.CurveList" />.  The ClusterScaleWidth is set automatically
		/// each time <see c_ref="GraphPane.AxisChange()" /> is called.
		/// </remarks>
		/// <seealso c_ref="Default.ClusterScaleWidth"/>
		/// <seealso c_ref="ClusterScaleWidthAuto"/>
		/// <seealso c_ref="MinBarGap"/>
...
//			else if ( minStep <= 0 || minStep < 0.001 * range || minStep > range )
		...
		/// </summary>
		/// <remarks>This method calls the <see c_ref="Scale.GetClusterWidth(GraphPane)"/>
		/// method for the base <see c_ref="Axis"/> for <see c_ref="Bar"/> graphs
		/// (the base <see c_ref="Axis"/> is assigned by the <see c_ref="ZedGraph.BarSettings.Base"/>
		/// property).
		/// </remarks>
		/// <seealso c_ref="ZedGraph.BarBase"/>
		/// <seealso c_ref="ZedGraph.BarSettings"/>
		/// <seealso c_ref="Scale.GetClusterWidth(GraphPane)"/>
		/// <seealso c_ref="ZedGraph.BarSettings.Type"/>
		...
		/// <summary>
		/// Determine the <see c_ref="Axis"/> from which the <see c_ref="Bar"/> charts are based.
		/// </summary>
		/// <seealso c_ref="ZedGraph.BarBase"/>
		/// <seealso c_ref="BarSettings"/>
		/// <seealso c_ref="ZedGraph.BarSettings.Base"/>
		/// <seealso c_ref="Scale.GetClusterWidth(GraphPane)"/>
		/// <returns>The <see c_ref="Axis"/> class for the axis from which the bars are based</returns>
	...
	#region Defaults
		...
		/// default property values for the <see c_ref="BarSettings"/> class.
		/// </summary>
		public struct Default
		{
			...
			/// </summary>
			/// <seealso c_ref="Default.MinBarGap"/>
			/// <seealso c_ref="BarSettings.MinClusterGap"/>
			public static float MinClusterGap = 1.0F;
			/// <summary>
			/// The default dimension gap between each individual bar within a bar cluster
			/// on a <see c_ref="Bar"/> graph.
			/// This dimension is expressed in terms of the normal bar width.
			/// </summary>
			/// <seealso c_ref="Default.MinClusterGap"/>
			/// <seealso c_ref="BarSettings.MinBarGap"/>
			public static float MinBarGap = 0.2F;
			/// <summary>The default value for the <see c_ref="BarSettings.Base"/>, which determines the base
			/// <see c_ref="Axis"/> from which the <see c_ref="Bar"/> graphs will be displayed.
			/// </summary>
			/// <seealso c_ref="BarSettings.Base"/>
			public static BarBase Base = BarBase.X;
			/// <summary>The default value for the <see c_ref="BarSettings.Type"/> property, which
			/// determines if the bars are drawn overlapping eachother in a "stacked" format,
			/// or side-by-side in a "cluster" format.  See the <see c_ref="ZedGraph.BarType"/>
			...
			/// <summary>
			/// The default value for <see c_ref="BarSettings.ClusterScaleWidthAuto" />.
			/// </summary>
