...
namespace UIGraphLib
{
	...
	/// <see c_ref="LinearScale"/>, <see c_ref="LogScale"/>, <see c_ref="OrdinalScale"/>,
	/// <see c_ref="TextScale"/>, <see c_ref="DateScale"/>, <see c_ref="ExponentScale"/>,
	/// <see c_ref="DateAsOrdinalScale"/>, and <see c_ref="LinearAsOrdinalScale"/>
	...
	#region Fields
		...
		/// <summary> Private fields for the <see c_ref="Axis"/> scale definitions.
		/// Use the public properties <see c_ref="Min"/>, <see c_ref="Max"/>,
		/// <see c_ref="MajorStep"/>, <see c_ref="MinorStep"/>, and <see c_ref="Exponent" />
		...
		/// <summary> Private fields for the <see c_ref="Axis"/> automatic scaling modes.
		/// Use the public properties <see c_ref="MinAuto"/>, <see c_ref="MaxAuto"/>,
		/// <see c_ref="MajorStepAuto"/>, <see c_ref="MinorStepAuto"/>, 
		/// <see c_ref="MagAuto"/> and <see c_ref="FormatAuto"/>
		/// for access to these values.
		/// </summary>
		...
		/// <summary> Private fields for the <see c_ref="Axis"/> "grace" settings.
		/// These values determine how much extra space is left before the first data value
		/// and after the last data value.
		/// Use the public properties <see c_ref="MinGrace"/> and <see c_ref="MaxGrace"/>
		/// for access to these values.
		/// </summary>
		...
		/// <summary> Private field for the <see c_ref="Axis"/> scale value display.
		/// Use the public property <see c_ref="Mag"/> for access to this value.
		/// </summary>
		internal int		_mag;

		/// <summary> Private fields for the <see c_ref="Scale"/> attributes.
		/// Use the public properties <see c_ref="Scale.IsReverse"/> and <see c_ref="Scale.IsUseTenPower"/>
		/// for access to these values.
		/// </summary>
		...
		/// <seealso c_ref="FormatAuto"/>
		internal string	_format;

		/// <summary>
		/// Private fields for Unit types to be used for the major and minor tics.
		/// See <see c_ref="MajorUnit"/> and <see c_ref="MinorUnit"/> for the corresponding
		/// public properties.
		/// These types only apply for date-time scales (<see c_ref="IsDate"/>).
		/// </summary>
		/// <value>The value of these types is of enumeration type <see c_ref="DateUnit"/>
		/// </value>
		internal DateUnit		_majorUnit,
									_minorUnit;

		/// <summary> Private field for the alignment of the <see c_ref="Axis"/> tic labels.
		/// This fields controls whether the inside, center, or outside edges of the text labels are aligned.
		/// Use the public property <see c_ref="Scale.Align"/>
		...
		/// Use the public property <see c_ref="Scale.AlignH"/>
		/// for access to this value. </summary>
		/// <seealso c_ref="FormatAuto"/>
		internal AlignH _alignH;


		/// <summary> Private fields for the <see c_ref="Axis"/> font specificatios.
		/// Use the public properties <see c_ref="FontSpec"/> and
		/// <see c_ref="Scale.FontSpec"/> for access to these values. </summary>
		internal FontSpec _fontSpec;

		/// <summary>
		/// Internal field that stores the amount of space between the scale labels and the
		/// major tics.  Use the public property <see c_ref="LabelGap" /> to access this
		/// value.
		/// </summary>
		...
		/// </summary>
		/// <remarks>
		/// These values are just <see c_ref="Scale.Min" /> and <see c_ref="Scale.Max" />
		/// for normal linear scales, but for log or exponent scales they will be a
		/// linear representation.  For <see c_ref="LogScale" />, it is the
		/// <see c_ref="Math.Log(double)" /> of the value, and for <see c_ref="ExponentScale" />,
		/// it is the <see c_ref="Math.Exp(double)" />
		/// of the value.
		/// </remarks>
		...
		/// <summary>
		/// Gets or sets the linearized version of the <see c_ref="Min" /> scale range.
		/// </summary>
		/// <remarks>
		/// This value is valid at any time, whereas <see c_ref="_minLinTemp" /> is an optimization
		...
		/// <summary>
		/// Gets or sets the linearized version of the <see c_ref="Max" /> scale range.
		/// </summary>
		/// <remarks>
		/// This value is valid at any time, whereas <see c_ref="_maxLinTemp" /> is an optimization
		/// pre-set that is only valid during draw operations.
		/// </remarks>
	...
	#region Defaults
		...
		/// default property values for the <see c_ref="Scale"/> class.
		/// </summary>
		public struct Default
		{
			/// <summary>
			/// The default "zero lever" for automatically selecting the axis
			/// scale range (see <see c_ref="PickScale"/>). This number is
			...
			/// <see c_ref="Default"/> class, and cannot be changed after compilation.
			/// </summary>
			...
			/// </summary>
			/// <seealso c_ref="MinGrace"/>
			/// <seealso c_ref="MaxGrace"/>
			public static double MaxGrace = 0.1;
			/// <summary>
			/// The maximum number of text labels (major tics) that will be allowed on the plot by
			/// the automatic scaling logic.  This value applies only to <see c_ref="AxisType.Text"/>
			/// axes.  If there are more than MaxTextLabels on the plot, then
			/// <see c_ref="MajorStep"/> will be increased to reduce the number of labels.  That is,
			...
			/// scale step size (see <see c_ref="PickScale"/>).
			...
			/// scale minor step size (see <see c_ref="PickScale"/>).
			...
			/// <summary>
			/// The default reverse mode for the <see c_ref="Axis"/> scale
			/// (<see c_ref="IsReverse"/> property). true for a reversed scale
			/// (X decreasing to the left, Y/Y2 decreasing upwards), false otherwise.
			/// </summary>
			public static bool IsReverse = false;
			/// <summary>
			/// The default setting for the <see c_ref="Axis"/> scale format string
			/// (<see c_ref="Format"/> property).  For numeric values, this value is
			/// setting according to the <see c_ref="String.Format(string,object)"/> format strings.  For date
			/// type values, this value is set as per the <see c_ref="XDate.ToString()"/> function.
			/// </summary>
			//public static string ScaleFormat = "&dd-&mmm-&yy &hh:&nn";
			public static string Format = "g";
			/// <summary>
			/// A default setting for the <see c_ref="AxisType.Date"/> auto-ranging code.
			/// This values applies only to Date-Time type axes.
			/// If the total span of data exceeds this number (in days), then the auto-range
			/// code will select <see c_ref="MajorUnit"/> = <see c_ref="DateUnit.Year"/>
			/// and <see c_ref="MinorUnit"/> = <see c_ref="DateUnit.Year"/>.
			/// This value normally defaults to 1825 days (5 years).
			/// This value is used by the <see c_ref="DateScale.CalcDateStepSize"/> method.
			/// </summary>
			...
			/// If the total span of data exceeds this number (in days), then the auto-range
			/// code will select <see c_ref="MajorUnit"/> = <see c_ref="DateUnit.Year"/>
			/// and <see c_ref="MinorUnit"/> = <see c_ref="DateUnit.Month"/>.
			...
			/// This values applies only to Date-Time type axes.
			/// If the total span of data exceeds this number (in days), then the auto-range
			/// code will select <see c_ref="MajorUnit"/> = <see c_ref="DateUnit.Month"/>
			/// and <see c_ref="MinorUnit"/> = <see c_ref="DateUnit.Month"/>.
			...
			/// This values applies only to Date-Time type axes.
			/// If the total span of data exceeds this number (in days), then the auto-range
			/// code will select <see c_ref="MajorUnit"/> = <see c_ref="DateUnit.Day"/>
			/// and <see c_ref="MinorUnit"/> = <see c_ref="DateUnit.Day"/>.
			...
			/// If the total span of data exceeds this number (in days), then the auto-range
			/// code will select <see c_ref="MajorUnit"/> = <see c_ref="DateUnit.Day"/>
			/// and <see c_ref="MinorUnit"/> = <see c_ref="DateUnit.Hour"/>.
			...
			/// This values applies only to Date-Time type axes.
			/// If the total span of data exceeds this number (in days), then the auto-range
			/// code will select <see c_ref="MajorUnit"/> = <see c_ref="DateUnit.Hour"/>
			/// and <see c_ref="MinorUnit"/> = <see c_ref="DateUnit.Hour"/>.
			...
			/// If the total span of data exceeds this number (in days), then the auto-range
			/// code will select <see c_ref="MajorUnit"/> = <see c_ref="DateUnit.Hour"/>
			/// and <see c_ref="MinorUnit"/> = <see c_ref="DateUnit.Minute"/>.
			...
			/// This values applies only to Date-Time type axes.
			/// If the total span of data exceeds this number (in days), then the auto-range
			/// code will select <see c_ref="MajorUnit"/> = <see c_ref="DateUnit.Minute"/>
			/// and <see c_ref="MinorUnit"/> = <see c_ref="DateUnit.Minute"/>.
			...
			/// If the total span of data exceeds this number (in days), then the auto-range
			/// code will select <see c_ref="MajorUnit"/> = <see c_ref="DateUnit.Minute"/>
			/// and <see c_ref="MinorUnit"/> = <see c_ref="DateUnit.Second"/>.
			...
			/// This values applies only to Date-Time type axes.
			/// If the total span of data exceeds this number (in days), then the auto-range
			/// code will select <see c_ref="MajorUnit"/> = <see c_ref="DateUnit.Second"/>
			/// and <see c_ref="MinorUnit"/> = <see c_ref="DateUnit.Second"/>.
			...
			/// selects a <see c_ref="Format"/> of <see c_ref="DateUnit.Year"/>
			/// for <see c_ref="MajorUnit"/> and <see c_ref="DateUnit.Year"/> for 
			/// for <see c_ref="MinorUnit"/>.
			/// This value is used by the <see c_ref="DateScale.CalcDateStepSize"/> method.
			/// </summary>
			/// <seealso c_ref="System.Globalization.DateTimeFormatInfo"/>
			...
			/// This is the format used for the scale values when auto-ranging code
			/// selects a <see c_ref="Format"/> of <see c_ref="DateUnit.Year"/>
			/// for <see c_ref="MajorUnit"/> and <see c_ref="DateUnit.Month"/> for 
			/// for <see c_ref="MinorUnit"/>.
			/// This value is used by the <see c_ref="DateScale.CalcDateStepSize"/> method.
			/// </summary>
			/// <seealso c_ref="System.Globalization.DateTimeFormatInfo"/>
			...
			/// This values applies only to Date-Time type axes.
			/// This is the format used for the scale values when auto-ranging code
			/// selects a <see c_ref="Format"/> of <see c_ref="DateUnit.Month"/>
			/// for <see c_ref="MajorUnit"/> and <see c_ref="DateUnit.Month"/> for 
			/// for <see c_ref="MinorUnit"/>.
			/// This value is used by the <see c_ref="DateScale.CalcDateStepSize"/> method.
			/// </summary>
			...
			/// This values applies only to Date-Time type axes.
			/// This is the format used for the scale values when auto-ranging code
			/// selects a <see c_ref="Format"/> of <see c_ref="DateUnit.Day"/>
			/// for <see c_ref="MajorUnit"/> and <see c_ref="DateUnit.Day"/> for 
			/// for <see c_ref="MinorUnit"/>.
			/// This value is used by the <see c_ref="DateScale.CalcDateStepSize"/> method.
			/// </summary>
			/// <seealso c_ref="System.Globalization.DateTimeFormatInfo"/>
			...
			/// This is the format used for the scale values when auto-ranging code
			/// selects a <see c_ref="Format"/> of <see c_ref="DateUnit.Day"/>
			/// for <see c_ref="MajorUnit"/> and <see c_ref="DateUnit.Hour"/> for 
			/// for <see c_ref="MinorUnit"/>.
			/// This value is used by the <see c_ref="DateScale.CalcDateStepSize"/> method.
			/// </summary>
			/// <seealso c_ref="System.Globalization.DateTimeFormatInfo"/>
			...
			/// This values applies only to Date-Time type axes.
			/// This is the format used for the scale values when auto-ranging code
			/// selects a <see c_ref="Format"/> of <see c_ref="DateUnit.Hour"/>
			/// for <see c_ref="MajorUnit"/> and <see c_ref="DateUnit.Hour"/> for 
			/// for <see c_ref="MinorUnit"/>.
			/// This value is used by the <see c_ref="DateScale.CalcDateStepSize"/> method.
			/// </summary>
			...
			/// This is the format used for the scale values when auto-ranging code
			/// selects a <see c_ref="Format"/> of <see c_ref="DateUnit.Hour"/>
			/// for <see c_ref="MajorUnit"/> and <see c_ref="DateUnit.Minute"/> for 
			/// for <see c_ref="MinorUnit"/>.
			/// This value is used by the <see c_ref="DateScale.CalcDateStepSize"/> method.
			/// </summary>
			/// <seealso c_ref="System.Globalization.DateTimeFormatInfo"/>
			...
			/// This values applies only to Date-Time type axes.
			/// This is the format used for the scale values when auto-ranging code
			/// selects a <see c_ref="Format"/> of <see c_ref="DateUnit.Minute"/>
			/// for <see c_ref="MajorUnit"/> and <see c_ref="DateUnit.Minute"/> for 
			/// for <see c_ref="MinorUnit"/>.
			/// This value is used by the <see c_ref="DateScale.CalcDateStepSize"/> method.
			/// </summary>
			...
			/// This is the format used for the scale values when auto-ranging code
			/// selects a <see c_ref="Format"/> of <see c_ref="DateUnit.Minute"/>
			/// for <see c_ref="MajorUnit"/> and <see c_ref="DateUnit.Second"/> for 
			/// for <see c_ref="MinorUnit"/>.
			/// This value is used by the <see c_ref="DateScale.CalcDateStepSize"/> method.
			/// </summary>
			/// <seealso c_ref="System.Globalization.DateTimeFormatInfo"/>
			...
			/// This values applies only to Date-Time type axes.
			/// This is the format used for the scale values when auto-ranging code
			/// selects a <see c_ref="Format"/> of <see c_ref="DateUnit.Second"/>
			/// for <see c_ref="MajorUnit"/> and <see c_ref="DateUnit.Second"/> for 
			/// for <see c_ref="MinorUnit"/>.
			/// This value is used by the <see c_ref="DateScale.CalcDateStepSize"/> method.
			/// </summary>
			...
			/// This values applies only to Date-Time type axes.
			/// This is the format used for the scale values when auto-ranging code
			/// selects a <see c_ref="Format"/> of <see c_ref="DateUnit.Millisecond"/>
			/// for <see c_ref="MajorUnit"/> and <see c_ref="DateUnit.Millisecond"/> for 
			/// for <see c_ref="MinorUnit"/>.
			/// This value is used by the <see c_ref="DateScale.CalcDateStepSize"/> method.
			/// </summary>
			/// <seealso c_ref="System.Globalization.DateTimeFormatInfo"/>
			...
			*/
			...
			/// </summary>
			/// <seealso c_ref="AlignH"/>
			public static AlignH AlignH = AlignH.Center;
			/// <summary>
			/// The default font family for the <see c_ref="Axis"/> scale values
			/// font specification <see c_ref="FontSpec"/>
			/// (<see c_ref="ZedGraph.FontSpec.Family"/> property).
			/// </summary>
			public static string FontFamily = "Arial";
			/// <summary>
			/// The default font size for the <see c_ref="Axis"/> scale values
			/// font specification <see c_ref="FontSpec"/>
			...
			/// <summary>
			/// The default font color for the <see c_ref="Axis"/> scale values
			/// font specification <see c_ref="FontSpec"/>
			/// (<see c_ref="ZedGraph.FontSpec.FontColor"/> property).
			/// </summary>
			public static Color FontColor = Color.Black;
			/// <summary>
			/// The default font bold mode for the <see c_ref="Axis"/> scale values
			/// font specification <see c_ref="FontSpec"/>
			...
			/// <summary>
			/// The default font italic mode for the <see c_ref="Axis"/> scale values
			/// font specification <see c_ref="FontSpec"/>
			...
			/// <summary>
			/// The default font underline mode for the <see c_ref="Axis"/> scale values
			/// font specification <see c_ref="FontSpec"/>
			...
			/// <summary>
			/// The default value for <see c_ref="IsVisible"/>, which determines
			/// whether or not the scale values are displayed.
			/// </summary>
			public static bool IsVisible = true;
			/// <summary>
			/// The default value for <see c_ref="IsLabelsInside"/>, which determines
			/// whether or not the scale labels and title for the <see c_ref="Axis"/> will appear
			/// on the opposite side of the <see c_ref="Axis"/> that it normally appears.
			/// </summary>
			...
			/// omitted if the axis is shifted due to a non-default location using the <see c_ref="Axis.Cross"/>
			/// property.
			/// </summary>
			/// <remarks>
			/// This parameter applies only when <see c_ref="Axis.CrossAuto"/> is false.  It is scaled according
			/// to the size of the graph based on <see c_ref="PaneBase.BaseDimension"/>.  When a non-default
...
*/
	...
	#region Serialization
		...
		/// <summary>
		/// Populates a <see c_ref="SerializationInfo"/> instance with the data needed to
	...
	#region properties
		...
		/// <summary>
		/// Get an <see c_ref="AxisType" /> enumeration that indicates the type of this scale.
		/// </summary>
		abstract public AxisType Type { get; }

		/// <summary>
		/// True if this scale is <see c_ref="AxisType.Log" />, false otherwise.
		/// </summary>
		public bool IsLog { get { return this is LogScale; } }
		/// <summary>
		/// True if this scale is <see c_ref="AxisType.Exponent" />, false otherwise.
		/// </summary>
		public bool IsExponent { get { return this is ExponentScale; } }
		/// <summary>
		/// True if this scale is <see c_ref="AxisType.Date" />, false otherwise.
		/// </summary>
		public bool IsDate { get { return this is DateScale; } }
		/// <summary>
		/// True if this scale is <see c_ref="AxisType.Text" />, false otherwise.
		/// </summary>
		public bool IsText { get { return this is TextScale; } }
		/// <summary>
		/// True if this scale is <see c_ref="AxisType.Ordinal" />, false otherwise.
		/// </summary>
		/// <remarks>
		/// Note that this is only true for an actual <see c_ref="OrdinalScale" /> class.
		/// This property will be false for other ordinal types such as
		/// <see c_ref="AxisType.Text" />, <see c_ref="AxisType.LinearAsOrdinal" />,
		/// or <see c_ref="AxisType.DateAsOrdinal" />.  Use the <see c_ref="IsAnyOrdinal" />
		/// as a "catchall" for all ordinal type axes.
		/// </remarks>
		public bool IsOrdinal { get { return this is OrdinalScale; } }

		/// <summary>
		/// Gets a value that indicates if this <see c_ref="Scale" /> is of any of the
		/// ordinal types in the <see c_ref="AxisType" /> enumeration.
		/// </summary>
		/// <seealso c_ref="Type" />
...
*/
		/// <summary>
		/// Gets or sets the minimum scale value for this <see c_ref="Scale" />.
		/// </summary>
		/// <remarks>This value can be set
		/// automatically based on the state of <see c_ref="MinAuto"/>.  If
		/// this value is set manually, then <see c_ref="MinAuto"/> will
		/// also be set to false.
		/// </remarks>
		/// <value> The value is defined in user scale units for <see c_ref="AxisType.Log"/>
		/// and <see c_ref="AxisType.Linear"/> axes. For <see c_ref="AxisType.Text"/>
		/// and <see c_ref="AxisType.Ordinal"/> axes,
		/// this value is an ordinal starting with 1.0.  For <see c_ref="AxisType.Date"/>
		/// axes, this value is in XL Date format (see <see c_ref="XDate"/>, which is the
		/// number of days since the reference date of January 1, 1900.</value>
		/// <seealso c_ref="Max"/>
		/// <seealso c_ref="MajorStep"/>
		/// <seealso c_ref="MinorStep"/>
		/// <seealso c_ref="MinAuto"/>
		...
		/// <summary>
		/// Gets or sets the maximum scale value for this <see c_ref="Scale" />.
		/// </summary>
		/// <remarks>
		/// This value can be set
		/// automatically based on the state of <see c_ref="MaxAuto"/>.  If
		/// this value is set manually, then <see c_ref="MaxAuto"/> will
		/// also be set to false.
		/// </remarks>
		/// <value> The value is defined in user scale units for <see c_ref="AxisType.Log"/>
		/// and <see c_ref="AxisType.Linear"/> axes. For <see c_ref="AxisType.Text"/>
		...
		/// <summary>
		/// Gets or sets the scale step size for this <see c_ref="Scale" /> (the increment between
		/// labeled axis values).
		/// </summary>
		/// <remarks>
		/// This value can be set
		/// automatically based on the state of <see c_ref="MajorStepAuto"/>.  If
		/// this value is set manually, then <see c_ref="MajorStepAuto"/> will
		/// also be set to false.  This value is ignored for <see c_ref="AxisType.Log"/>
		/// axes.  For <see c_ref="AxisType.Date"/> axes, this
		/// value is defined in units of <see c_ref="MajorUnit"/>.
		/// </remarks>
		/// <value> The value is defined in user scale units </value>
		/// <seealso c_ref="Min"/>
		/// <seealso c_ref="Max"/>
		/// <seealso c_ref="MinorStep"/>
		/// <seealso c_ref="MajorStepAuto"/>
		/// <seealso c_ref="ZedGraph.Scale.Default.TargetXSteps"/>
		/// <seealso c_ref="ZedGraph.Scale.Default.TargetYSteps"/>
		/// <seealso c_ref="ZedGraph.Scale.Default.ZeroLever"/>
		/// <seealso c_ref="ZedGraph.Scale.Default.MaxTextLabels"/>
		...
		/// <summary>
		/// Gets or sets the scale minor step size for this <see c_ref="Scale" /> (the spacing between
		/// minor tics).
		/// </summary>
		/// <remarks>This value can be set
		/// automatically based on the state of <see c_ref="MinorStepAuto"/>.  If
		/// this value is set manually, then <see c_ref="MinorStepAuto"/> will
		/// also be set to false.  This value is ignored for <see c_ref="AxisType.Log"/> and
		/// <see c_ref="AxisType.Text"/> axes.  For <see c_ref="AxisType.Date"/> axes, this
		/// value is defined in units of <see c_ref="MinorUnit"/>.
		/// </remarks>
		/// <value> The value is defined in user scale units </value>
		/// <seealso c_ref="Min"/>
		/// <seealso c_ref="Max"/>
		/// <seealso c_ref="MajorStep"/>
		/// <seealso c_ref="MinorStepAuto"/>
		...
		/// <summary>
		/// Gets or sets the scale exponent value.  This only applies to <see c_ref="AxisType.Exponent" />. 
		/// </summary>
		/// <seealso c_ref="Min"/>
		/// <seealso c_ref="Max"/>
		/// <seealso c_ref="MinorStep"/>
		...
		/// For example, on a scale range with <see c_ref="Min"/> = 0, <see c_ref="Max"/> = 1000,
		/// and <see c_ref="MajorStep"/> = 200, a <see c_ref="BaseTic"/> value of 50 would cause
		/// the scale labels to appear at values 50, 250, 450, 650, and 850.  Note that the
		/// default value for this property is <see c_ref="PointPairBase.Missing"/>, which means the
		/// value is not used.  Setting this property to any value other than
		/// <see c_ref="PointPairBase.Missing"/> will activate the effect.  The value specified must
		/// coincide with the first major tic.  That is, if <see c_ref="BaseTic"/> were set to
		...
		/// <see c_ref="BaseTic"/>.  That is, in the example above, if the <see c_ref="MinorStep"/>
		...
		/// <summary>
		/// Gets or sets the type of units used for the major step size (<see c_ref="MajorStep"/>).
		/// </summary>
		/// <remarks>
		/// This unit type only applies to Date-Time axes (<see c_ref="AxisType.Date"/> = true).
		/// The axis is set to date type with the <see c_ref="Type"/> property.
		/// The unit types are defined as <see c_ref="DateUnit"/>.
		/// </remarks>
		/// <value> The value is a <see c_ref="DateUnit"/> enum type </value>
		/// <seealso c_ref="Min"/>
		/// <seealso c_ref="Max"/>
		/// <seealso c_ref="MajorStep"/>
		/// <seealso c_ref="MinorStep"/>
		...
		/// <summary>
		/// Gets or sets the type of units used for the minor step size (<see c_ref="MinorStep"/>).
		/// </summary>
		/// <remarks>
		/// This unit type only applies to Date-Time axes (<see c_ref="AxisType.Date"/> = true).
		/// The axis is set to date type with the <see c_ref="Type"/> property.
		...
		/// <see c_ref="MajorStep" /> to match the units of <see c_ref="Min" />
		/// and <see c_ref="Max" />.  This reflects the setting of
		/// <see c_ref="MajorUnit" />.
		/// </remarks>
		...
		/// <see c_ref="MinorStep" /> to match the units of <see c_ref="Min" />
		/// and <see c_ref="Max" />.  This reflects the setting of
		/// <see c_ref="MinorUnit" />.
		/// </remarks>
		...
		/// <summary>
		/// Gets or sets a value that determines whether or not the minimum scale value <see c_ref="Min"/>
		/// is set automatically.
		/// </summary>
		/// <remarks>
		/// This value will be set to false if
		/// <see c_ref="Min"/> is manually changed.
		/// </remarks>
		...
		/// <summary>
		/// Gets or sets a value that determines whether or not the maximum scale value <see c_ref="Max"/>
		/// is set automatically.
		/// </summary>
		/// <remarks>
		/// This value will be set to false if
		/// <see c_ref="Max"/> is manually changed.
		/// </remarks>
		/// <value>true for automatic mode, false for manual mode</value>
		/// <seealso c_ref="Max"/>
		public bool MaxAuto
		...
		/// <summary>
		/// Gets or sets a value that determines whether or not the scale step size <see c_ref="MajorStep"/>
		/// is set automatically.
		/// </summary>
		/// <remarks>
		/// This value will be set to false if
		/// <see c_ref="MajorStep"/> is manually changed.
		/// </remarks>
		/// <value>true for automatic mode, false for manual mode</value>
		/// <seealso c_ref="MajorStep"/>
		...
		/// <summary>
		/// Gets or sets a value that determines whether or not the minor scale step size <see c_ref="MinorStep"/>
		/// is set automatically.
		/// </summary>
		/// <remarks>
		/// This value will be set to false if
		/// <see c_ref="MinorStep"/> is manually changed.
		/// </remarks>
		/// <value>true for automatic mode, false for manual mode</value>
		/// <seealso c_ref="MinorStep"/>
		...
		/// <summary>
		/// Determines whether or not the scale label format <see c_ref="Format"/>
		/// is determined automatically based on the range of data values.
		/// </summary>
		/// <remarks>
		/// This value will be set to false if
		/// <see c_ref="Format"/> is manually changed.
		/// </remarks>
		/// <value>true if <see c_ref="Format"/> will be set automatically, false
		/// if it is to be set manually by the user</value>
		/// <seealso c_ref="Mag"/>
		/// <seealso c_ref="Format"/>
		/// <seealso c_ref="FontSpec"/>
		...
		/// <summary>
		/// The format of the <see c_ref="Axis"/> tic labels.
		/// </summary>
		/// <remarks>
		/// This property may be a date format or a numeric format, depending on the setting of
		/// <see c_ref="Type">Scale.Type</see>.
		/// This property may be set automatically by ZedGraph, depending on the state of
		/// <see c_ref="FormatAuto"/>.
		/// </remarks>
		/// <value>The format string conforms to the
		/// <see c_ref="System.Globalization.DateTimeFormatInfo" /> for date formats, and
		/// <see c_ref="System.Globalization.NumberFormatInfo" /> for numeric formats.
		/// </value>
		/// <seealso c_ref="Mag"/>
		/// <seealso c_ref="FormatAuto"/>
		/// <seealso c_ref="FontSpec"/>
		// /// <seealso c_ref="NumDec"/>
		...
		/// depending on the state of <see c_ref="MagAuto"/>.
		/// If this value is set manually by the user,
		/// then <see c_ref="MagAuto"/> will also be set to false.
		/// </remarks>
		/// <value>The magnitude multiplier (power of 10) for the scale
		/// value labels</value>
		/// <seealso c_ref="AxisLabel.IsOmitMag"/>
		/// <seealso c_ref="Axis.Title"/>
		/// <seealso c_ref="Format"/>
		/// <seealso c_ref="FontSpec"/>
		// /// <seealso c_ref="NumDec"/>
		public int Mag
		...
		/// <summary>
		/// Determines whether the <see c_ref="Mag"/> value will be set
		/// automatically based on the data, or manually by the user.
		/// </summary>
		/// <remarks>
		/// If the user manually sets the <see c_ref="Mag"/> value, then this
		/// flag will be set to false.
		/// </remarks>
		/// <value>true to have <see c_ref="Mag"/> set automatically,
		...
		/// </remarks>
		/// <seealso c_ref="Min"/>
		/// <seealso c_ref="ZedGraph.Scale.Default.MinGrace"/>
		/// <seealso c_ref="MaxGrace"/>
		...
		/// </remarks>
		/// <seealso c_ref="Max"/>
		/// <seealso c_ref="ZedGraph.Scale.Default.MaxGrace"/>
		/// <seealso c_ref="MinGrace"/>
		...
		/// <summary> Controls the alignment of the <see c_ref="Axis"/> tic labels.
		/// </summary>
		/// <remarks>
		...
		/// <summary>
		/// Gets a reference to the <see c_ref="ZedGraph.FontSpec"/> class used to render
		/// the scale values
		/// </summary>
		...
		/// </summary>
		/// <remarks>
		/// For example, setting this flag to true for the <see c_ref="YAxis"/> will shift the
		/// axis labels and title to the right side of the <see c_ref="YAxis"/> instead of the
		/// normal left-side location.  Set this property to true for the <see c_ref="XAxis" />,
		/// and set the <see c_ref="Axis.Cross"/> property for the <see c_ref="XAxis"/> to an arbitrarily
		/// large value (assuming <see c_ref="IsReverse"/> is false for the <see c_ref="YAxis" />) in
		/// order to have the <see c_ref="XAxis"/> appear at the top of the <see c_ref="Chart.Rect" />.
		/// </remarks>
		/// <seealso c_ref="IsReverse"/>
		/// <seealso c_ref="Axis.Cross"/>
		...
		/// <summary>
		/// Gets or sets a value that causes the first scale label for this <see c_ref="Axis"/> to be
		/// hidden.
		/// </summary>
		/// <remarks>
		/// Often, for axis that have an active <see c_ref="Axis.Cross"/> setting (e.g., <see c_ref="Axis.CrossAuto"/>
		...
		/// to true will hide any scale label that appears within <see c_ref="Scale.Default.EdgeTolerance"/> of the
		/// beginning of the <see c_ref="Axis"/>.
		/// </remarks>
		...
		/// <summary>
		/// Gets or sets a value that causes the last scale label for this <see c_ref="Axis"/> to be
		/// hidden.
		/// </summary>
		/// <remarks>
		/// Often, for axis that have an active <see c_ref="Axis.Cross"/> setting (e.g., <see c_ref="Axis.CrossAuto"/>
		...
		/// <summary>
		/// Gets or sets a value that causes the scale label that is located at the <see c_ref="Axis.Cross" />
		/// value for this <see c_ref="Axis"/> to be hidden.
		/// </summary>
		/// <remarks>
		/// For axes that have an active <see c_ref="Axis.Cross"/> setting (e.g., <see c_ref="Axis.CrossAuto"/>
		/// is false), the scale label at the <see c_ref="Axis.Cross" /> value is overlapped by opposing axes.
		...
		/// <summary>
		/// Determines if the scale values are reversed for this <see c_ref="Axis"/>
		/// </summary>
		/// <value>true for the X values to decrease to the right or the Y values to
		/// decrease upwards, false otherwise</value>
		/// <seealso c_ref="ZedGraph.Scale.Default.IsReverse"/>.
		...
		/// <see c_ref="IsLog"/> and <see c_ref="Type"/>).
		/// </remarks>
		...
		/// <summary>
		/// Gets or sets a <see c_ref="bool"/> value that determines if ZedGraph will check to
		/// see if the <see c_ref="Axis"/> scale labels are close enough to overlap.  If so,
		/// ZedGraph will adjust the step size to prevent overlap.
		/// </summary>
		/// <remarks>
		/// The process of checking for overlap is done during the <see c_ref="GraphPane.AxisChange()"/>
		/// method call, and affects the selection of the major step size (<see c_ref="MajorStep"/>).
		/// </remarks>
		...
		/// <summary>
		/// The text labels for this <see c_ref="Axis"/>.
		/// </summary>
		/// <remarks>
		/// This property is only
		/// applicable if <see c_ref="Type"/> is set to <see c_ref="AxisType.Text"/>.
		/// </remarks>
...
*/
		...
		/// for <see c_ref="AxisType.Log" /> and <see c_ref="AxisType.Exponent" />,
...
/*
		...
		/// </summary>
		/// <remarks>
		/// This method properly accounts for <see c_ref="IsLog"/>, <see c_ref="IsText"/>,
		...
		/// </param>
		/// <param name="dVal">
		/// The numeric value associated with the label.  This value is ignored for log (<see c_ref="IsLog"/>)
		/// and text (<see c_ref="IsText"/>) type axes.
		/// </param>
...
*/
		...
		/// true to get the bounding box of the text using the <see c_ref="ZedGraph.FontSpec.Angle" />,
		...
		/// value of <see c_ref="Axis.Cross"/>.  A positive value is into the ChartRect relative to
...
//			_ownerAxis.DrawMinorTics( g, pane, baseVal, shiftPos, scaleFactor, topPix );
		...
		/// </summary>
		/// <remarks>
		/// This method uses the <see c_ref="BarSettings.ClusterScaleWidth" /> for
	...
	#region Scale Picker Methods
		...
		/// routine honors the <see c_ref="MinAuto"/>, <see c_ref="MaxAuto"/>,
		/// and <see c_ref="MajorStepAuto"/> autorange settings as well as the <see c_ref="IsLog"/>
		/// setting.  In the event that any of the autorange settings are false, the
		/// corresponding <see c_ref="Min"/>, <see c_ref="Max"/>, or <see c_ref="MajorStep"/>
		/// setting is explicitly honored, and the remaining autorange settings (if any) will
		/// be calculated to accomodate the non-autoranged values.  The basic defaults for
		/// scale selection are defined using <see c_ref="Default.ZeroLever"/>,
		/// <see c_ref="Default.TargetXSteps"/>, and <see c_ref="Default.TargetYSteps"/>
		/// from the <see c_ref="Default"/> default class.
		/// <para>On Exit:</para>
		/// <para><see c_ref="Min"/> is set to scale minimum (if <see c_ref="MinAuto"/> = true)</para>
		/// <para><see c_ref="Max"/> is set to scale maximum (if <see c_ref="MaxAuto"/> = true)</para>
		/// <para><see c_ref="MajorStep"/> is set to scale step size (if <see c_ref="MajorStepAuto"/> = true)</para>
		/// <para><see c_ref="MinorStep"/> is set to scale minor step size (if <see c_ref="MinorStepAuto"/> = true)</para>
		/// <para><see c_ref="Mag"/> is set to a magnitude multiplier according to the data</para>
		/// <para><see c_ref="Format"/> is set to the display format for the values (this controls the
		...
		/// <remarks>
		/// This utility method
		/// will try to honor the <see c_ref="Default.TargetXSteps"/> and
		/// <see c_ref="Default.TargetYSteps"/> number of
		/// steps while using a rational increment (1, 2, or 5 -- which are
		/// even divisors of 10).  This method is used by <see c_ref="PickScale"/>.
		/// </remarks>
	...
	#region Coordinate Transform Methods
		...
		/// account the scale range (<see c_ref="Min"/> and <see c_ref="Max"/>),
		/// logarithmic state (<see c_ref="IsLog"/>), scale reverse state
		/// (<see c_ref="IsReverse"/>) and axis type (<see c_ref="XAxis"/>,
		/// <see c_ref="YAxis"/>, or <see c_ref="Y2Axis"/>).
		/// Note that the <see c_ref="Chart.Rect"/> must be valid, and
		/// <see c_ref="SetupScaleData"/> must be called for the
		/// current configuration before using this method (this is called everytime
		/// the graph is drawn (i.e., <see c_ref="GraphPane.Draw"/> is called).
		/// </remarks>
		...
		/// for use in calling the <see c_ref="Graphics"/> draw routines</returns>
		...
		/// this is an <see c_ref="AxisType.Ordinal"/> axis</param>
		/// <param name="x">The coordinate value, in user scale units, to
		/// be transformed</param>
		/// <returns>the coordinate value transformed to screen coordinates
		/// for use in calling the <see c_ref="Graphics"/> draw routines</returns>
		...
		/// logarithmic state (<see c_ref="IsLog"/>), scale reverse state
		/// (<see c_ref="IsReverse"/>) and axis type (<see c_ref="XAxis"/>,
		/// <see c_ref="YAxis"/>, or <see c_ref="Y2Axis"/>).  Note that
		/// the <see c_ref="Chart.Rect"/> must be valid, and
		/// <see c_ref="SetupScaleData"/> must be called for the
		...
		/// be transformed</param>
		/// <returns>the coordinate value transformed to screen coordinates
		/// for use in calling the <see c_ref="Draw"/> method</returns>
