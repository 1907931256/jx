...
namespace UIGraphLib
{
	...
	#region Fields
		...
		/// <see c_ref="Line"/>.  Use the public
		/// property <see c_ref="IsSmooth"/> to access this value.
		/// </summary>
		private bool _isSmooth;
		/// <summary>
		/// Private field that stores the smoothing tension
		/// for this <see c_ref="Line"/>.  Use the public property
		/// <see c_ref="SmoothTension"/> to access this value.
		/// </summary>
		...
		/// smoothing.</value>
		/// <seealso c_ref="IsSmooth"/>
		/// <seealso c_ref="Default.IsSmooth"/>
		/// <seealso c_ref="Default.SmoothTension"/>
		private float _smoothTension;
		/// <summary>
		/// Private field that stores the <see c_ref="ZedGraph.StepType"/> for this
		/// <see c_ref="CurveItem"/>.  Use the public
		/// property <see c_ref="StepType"/> to access this value.
		/// </summary>
		private StepType _stepType;
		/// <summary>
		/// Private field that stores the <see c_ref="ZedGraph.Fill"/> data for this
		/// <see c_ref="Line"/>.  Use the public property <see c_ref="Fill"/> to
		/// access this value.
		/// </summary>
		private Fill _fill;
		/// <summary>
		/// Private field that determines if this <see c_ref="Line"/> will be drawn with
		/// optimizations enabled.  Use the public
		/// property <see c_ref="IsOptimizedDraw"/> to access this value.
		/// </summary>
	...
	#region Defaults
		...
		/// default property values for the <see c_ref="Line"/> class.
		/// </summary>
		new public struct Default
		{
			...
			/// <summary>
			/// The default value for the <see c_ref="Line.IsSmooth"/>
			/// property.
			/// </summary>
			public static bool IsSmooth = false;
			/// <summary>
			/// The default value for the <see c_ref="Line.SmoothTension"/> property.
			/// </summary>
			public static float SmoothTension = 0.5F;
			/// <summary>
			/// The default value for the <see c_ref="Line.IsOptimizedDraw"/> property.
			/// </summary>
			public static bool IsOptimizedDraw = false;

			/// <summary>
			/// Default value for the curve type property
			/// (<see c_ref="Line.StepType"/>).  This determines if the curve
			/// will be drawn by directly connecting the points from the
			/// <see c_ref="CurveItem.Points"/> data collection,
			...
			/// be forward oriented <code>ForwardStep</code> (<see c_ref="StepType"/>) or
			...
			/// </summary>
			/// <value><see c_ref="StepType"/> enum value</value>
	...
	#region Properties
		...
		/// for this <see c_ref="Line"/>.  This property is only used if
		/// <see c_ref="IsSmooth"/> is true.  A tension value 0.0 will just
		...
		/// <summary>
		/// Determines if the <see c_ref="CurveItem"/> will be drawn by directly connecting the
		/// points from the <see c_ref="CurveItem.Points"/> data collection,
		/// or if the curve will be a "stair-step" in which the points are
		/// connected by a series of horizontal and vertical lines that
		/// represent discrete, constant values.  Note that the values can
		/// be forward oriented <c>ForwardStep</c> (<see c_ref="ZedGraph.StepType"/>) or
		/// rearward oriented <c>RearwardStep</c>.
		/// That is, the points are defined at the beginning or end
		/// of the constant value for which they apply, respectively.
		/// The <see c_ref="StepType"/> property is ignored for lines
		/// that have <see c_ref="IsSmooth"/> set to true.
		/// </summary>
		/// <value><see c_ref="ZedGraph.StepType"/> enum value</value>
		/// <seealso c_ref="Default.StepType"/>
		...
		/// <summary>
		/// Gets or sets a boolean value that determines if this <see c_ref="Line"/> will be drawn with
		...
		/// explicitly enable it for each <see c_ref="LineItem.Line">LineItem.Line</see>.
	...
	#region Constructors
		...
		/// <summary>
		/// Default constructor that sets all <see c_ref="Line"/> properties to default
	...
	#region Rendering Methods
		...
		/// <summary>
		/// Render a single <see c_ref="Line"/> segment to the specified
			...
			out PointF[] arrPoints, out int count )
		{
			...
			if ( this.IsVisible && !this.Color.IsEmpty && points != null )
			{
                ...
                arrPoints = new PointF[(_stepType == UIGraphLib.StepType.NonStep ? 1 : 2) *
											points.Count + 1];
