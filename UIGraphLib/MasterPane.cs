...
namespace UIGraphLib
{
	...
	#region Fields
		...
		/// <summary>
		/// Private field that holds a collection of <see c_ref="GraphPane"/> objects for inclusion
		/// in this <see c_ref="MasterPane"/>.  Use the public property <see c_ref="PaneList"/>
		...
		/// <see c_ref="InnerPaneGap"/> to access this value;
		/// </summary>
		internal float _innerPaneGap;
		/// <summary>
		///Private field that stores a boolean value which signifies whether all 
		///<see c_ref="ZedGraph.GraphPane"/>s in the chart use the same entries in their 
		///<see c_ref="Legend"/>  If set to true, only one set of entries will be displayed in 
		///this <see c_ref="Legend"/> instance.  If set to false, this instance will display all 
		///entries from all <see c_ref="ZedGraph.GraphPane"/>s.
		/// </summary>
		private bool _isUniformLegendEntries;
		/// <summary>
		/// private field that determines if the
		/// <see c_ref="DoLayout(Graphics)" />
		/// function will automatically set
		/// the <see c_ref="PaneBase.BaseDimension" /> of each <see c_ref="GraphPane" /> in the
		/// <see c_ref="PaneList" /> such that the scale factors have the same value.
		/// </summary>
		private bool _isCommonScaleFactor;

		/// <summary>
		/// private field that saves the paneLayout format specified when
		/// <see c_ref="SetLayout(Graphics,PaneLayout)"/> was called. This value will
		/// default to <see c_ref="MasterPane.Default.PaneLayout"/> if
		/// <see c_ref="SetLayout(Graphics,PaneLayout)"/> (or an overload) was never called.
		/// </summary>
		internal PaneLayout _paneLayout;

		/// <summary>
		/// Private field that stores the boolean value that determines whether
		/// <see c_ref="_countList"/> is specifying rows or columns.
		/// </summary>
		internal bool _isColumnSpecified;
		/// <summary>
		/// private field that stores the row/column item count that was specified to the
		/// <see c_ref="SetLayout(Graphics,bool,int[],float[])"/> method.  This values will be
		/// null if <see c_ref="SetLayout(Graphics,bool,int[],float[])"/> was never called.
		/// </summary>
		internal int[] _countList;

		/// <summary>
		/// private field that stores the row/column size proportional values as specified
		/// to the <see c_ref="SetLayout(Graphics,bool,int[],float[])"/> method.  This
		/// value will be null if <see c_ref="SetLayout(Graphics,bool,int[],float[])"/>
		/// was never called.  
		/// </summary>
		internal float[] _prop;

/*		/// <summary>
		/// private field to store the <see c_ref="PaneLayoutMgr" /> instance, which
...
*/
	...
	#region Defaults
		...
		/// default property values for the <see c_ref="MasterPane"/> class.
		/// </summary>
		public new struct Default
		{
			...
			/// method calls.
			/// </summary>
			/// <seealso c_ref="SetLayout(Graphics,PaneLayout)" />
			/// <seealso c_ref="SetLayout(Graphics,int,int)" />
			/// <seealso c_ref="SetLayout(Graphics,bool,int[])" />
			/// <seealso c_ref="SetLayout(Graphics,bool,int[],float[])" />
			/// <seealso c_ref="ReSize(Graphics,RectangleF)" />
			public static PaneLayout PaneLayout = PaneLayout.SquareColPreferred;

			/// <summary>
			/// The default value for the <see c_ref="Default.InnerPaneGap"/> property.
			/// This is the size of the margin between adjacent <see c_ref="GraphPane"/>
			/// objects, in units of points (1/72 inch).
			/// </summary>
			/// <seealso c_ref="MasterPane.InnerPaneGap"/>
			public static float InnerPaneGap = 10;
			
			/// <summary>
			/// The default value for the <see c_ref="Legend.IsVisible"/> property for
			/// the <see c_ref="MasterPane"/> class.
			/// </summary>
			public static bool IsShowLegend = false;
			/// <summary>
			/// The default value for the <see c_ref="IsUniformLegendEntries"/> property.
			/// </summary>
			public static bool IsUniformLegendEntries = false;
			/// <summary>
			/// The default value for the <see c_ref="IsCommonScaleFactor"/> property.
			/// </summary>
	...
	#region Properties
		...
		/// <summary>
		/// Gets or sets the <see c_ref="PaneList"/> collection instance that holds the list of
		/// <see c_ref="GraphPane"/> objects that are included in this <see c_ref="MasterPane"/>.
		/// </summary>
		/// <seealso c_ref="Add"/>
		/// <seealso c_ref="MasterPane.this[int]"/>
...
/*
		/// <summary>
		/// Gets the <see c_ref="PaneLayoutMgr" /> instance, which manages the pane layout
		/// settings, and handles the layout functions.
		/// </summary>
		/// <seealso c_ref="ZedGraph.PaneLayoutMgr.SetLayout(PaneLayout)" />
		/// <seealso c_ref="ZedGraph.PaneLayoutMgr.SetLayout(int,int)" />
		/// <seealso c_ref="ZedGraph.PaneLayoutMgr.SetLayout(bool,int[])" />
		/// <seealso c_ref="ZedGraph.PaneLayoutMgr.SetLayout(bool,int[],float[])" />
		/// <seealso c_ref="ReSize" />
...
*/
		/// <summary>
		/// Gets or sets the size of the margin between adjacent <see c_ref="GraphPane"/>
		/// objects.
		/// </summary>
		/// <remarks>This property is scaled according to <see c_ref="PaneBase.CalcScaleFactor"/>,
		/// based on <see c_ref="PaneBase.BaseDimension"/>.  The default value comes from
		/// <see c_ref="Default.InnerPaneGap"/>.
		/// </remarks>
		...
		/// <summary>
		/// Gets or set the value of the	 <see c_ref="IsUniformLegendEntries"/>
		/// </summary>
		...
		/// <see c_ref="DoLayout(Graphics)" /> method will automatically set the
		/// <see c_ref="PaneBase.BaseDimension" />
		/// of each <see c_ref="GraphPane" /> in the <see c_ref="PaneList" /> such that the
		/// scale factors have the same value.
		/// </summary>
		/// <remarks>
		/// The scale factors, calculated by <see c_ref="PaneBase.CalcScaleFactor" />, determine
		...
		/// <see c_ref="MasterPane" /> will be forced to operate in Anti-alias mode.
		/// Note that if this value is set to "true", it overrides the setting for sub-objects.
		/// Otherwise, the sub-object settings (such as <see c_ref="FontSpec.IsAntiAlias"/>)
		/// will be honored.
		/// </summary>
	...
	#region Constructors
		...
		/// <summary>
		/// Default constructor for the class.  Sets the <see c_ref="PaneBase.Rect"/> to (0, 0, 500, 375).
		/// </summary>
		...
		/// calling the typed version of <see c_ref="Clone" /> to make a deep copy.
		/// </summary>
	...
	#region List Methods
		/// <summary>
		/// Indexer to access the specified <see c_ref="GraphPane"/> object from <see c_ref="PaneList"/>
		...
		/// <summary>
		/// Indexer to access the specified <see c_ref="GraphPane"/> object from <see c_ref="PaneList"/>
		/// by its <see c_ref="PaneBase.Title"/> string.
		/// </summary>
		...
		/// <summary>
		/// Add a <see c_ref="GraphPane"/> object to the <see c_ref="PaneList"/> collection at the end of the list.
		/// </summary>
		...
		/// <summary>
		/// Call <see c_ref="GraphPane.AxisChange()"/> for all <see c_ref="GraphPane"/> objects in the
		/// <see c_ref="PaneList"/> list.
		/// </summary>
		/// <remarks>
		...
		/// <summary>
		/// Redo the layout using the current size of the <see c_ref="PaneBase.Rect"/>,
		/// and also handle resizing the
		/// contents by calling <see c_ref="DoLayout(Graphics)"/>.
		/// </summary>
		/// <remarks>This method will use the pane layout that was specified by a call to
		/// <see c_ref="SetLayout(Graphics,PaneLayout)"/>.  If
		/// <see c_ref="SetLayout(Graphics,PaneLayout)"/> has not previously been called,
		/// it will default to <see c_ref="Default.PaneLayout"/>.
		/// </remarks>
		/// <param name="g">
		/// A graphic device object to be drawn into.  This is normally e.Graphics from the
		/// PaintEventArgs argument to the Paint() method.
		...
		/// <summary>
		/// Change the size of the <see c_ref="PaneBase.Rect"/>, and also handle resizing the
		/// contents by calling <see c_ref="DoLayout(Graphics)"/>.
		/// </summary>
		/// <remarks>This method will use the pane layout that was specified by a call to
		/// <see c_ref="SetLayout(Graphics,PaneLayout)"/>.  If
		...
		/// (via <see c_ref="PaneBase.CalcScaleFactor" />),
		/// to give a common scale factor for all <see c_ref="GraphPane" /> objects in the
		/// <see c_ref="PaneList" />.
		/// </summary>
		/// <remarks>
		/// This will make it such that a given font size will result in the same output font
		/// size for all <see c_ref="GraphPane" />'s.  Note that this does not make the scale
		/// factor for the <see c_ref="GraphPane" />'s the same as that of the
		/// <see c_ref="MasterPane" />.
		/// </remarks>
		/// <seealso c_ref="IsCommonScaleFactor" />
		...
		/// <summary>
		/// Render all the <see c_ref="GraphPane"/> objects in the <see c_ref="PaneList"/> to the
		/// specified graphics device.
		/// </summary>
		/// <remarks>This method should be part of the Paint() update process.  Calling this routine
		/// will redraw all
		/// features of all the <see c_ref="GraphPane"/> items.  No preparation is required other than
		/// instantiated <see c_ref="GraphPane"/> objects that have been added to the list with the
		/// <see c_ref="Add"/> method.
		/// </remarks>
		/// <param name="g">
		/// A graphic device object to be drawn into.  This is normally e.Graphics from the
		/// PaintEventArgs argument to the Paint() method.
		...
		/// </summary>
		/// <remarks>
		/// This method first finds the <see c_ref="GraphPane"/> within the list that contains
		/// the specified mouse point.  It then calls the <see c_ref="GraphPane.FindNearestObject"/>
		...
		/// in the <see c_ref="GraphPane.FindNearestObject"/> method.
		/// If the mouse point lies within the <see c_ref="PaneBase.Rect"/> of any 
		/// <see c_ref="GraphPane"/> item, then that pane will be returned (otherwise it will be
		...
		/// <see c_ref="GraphPane.Default.NearestTol"/> pixels), then the object will be returned.
		...
		/// PaintEventArgs argument to the Paint() method.
		/// </param>
		/// <param name="pane">A reference to the <see c_ref="GraphPane"/> object that was clicked.</param>
		...
		/// <returns>true if a <see c_ref="GraphPane"/> was found, false otherwise.</returns>
		...
		/// <summary>
		/// Find the <see c_ref="GraphPane"/> within the <see c_ref="PaneList"/> that contains the
		/// <see paramref="mousePt"/> within its <see c_ref="PaneBase.Rect"/>.
		/// </summary>
		/// <param name="mousePt">The mouse point location where you want to search</param>
		/// <returns>A <see c_ref="GraphPane"/> object that contains the mouse point, or
		/// null if no <see c_ref="GraphPane"/> was found.</returns>
		...
		/// <summary>
		/// Find the <see c_ref="GraphPane"/> within the <see c_ref="PaneList"/> that contains the
		/// <see paramref="mousePt"/> within its <see c_ref="Chart.Rect"/>.
		/// </summary>
		/// <param name="mousePt">The mouse point location where you want to search</param>
		/// <returns>A <see c_ref="GraphPane"/> object that contains the mouse point, or
		/// null if no <see c_ref="GraphPane"/> was found.</returns>
	...
	#region Layout Methods
		...
		/// <see c_ref="GraphPane" /> objects within a <see c_ref="MasterPane" />.  These functions
		/// do not make any changes, they merely set the parameters so that future calls
		/// to <see c_ref="PaneBase.ReSize" /> or <see c_ref="DoLayout(Graphics)" />
		/// will use the desired layout.<br /><br />
		/// The layout options include a set of "canned" layouts provided by the
		/// <see c_ref="ZedGraph.PaneLayout" /> enumeration, options to just set a specific
		...
		/// </overloads>
		/// <summary>
		/// Automatically set all of the <see c_ref="GraphPane"/> <see c_ref="PaneBase.Rect"/>'s in
		/// the list to a pre-defined layout configuration from a <see c_ref="PaneLayout" />
		/// enumeration.
		/// </summary>
		/// <remarks>This method uses a <see c_ref="PaneLayout"/> enumeration to describe the type of layout
		/// to be used.  Overloads are available that provide other layout options</remarks>
		/// <param name="paneLayout">A <see c_ref="PaneLayout"/> enumeration that describes how
		/// the panes should be laid out within the <see c_ref="PaneBase.Rect"/>.</param>
		/// <param name="g">
		...
		/// in the layout, and all <see c_ref="GraphPane" /> objects will have the same size.
		/// Overloads are available that provide other layout options</remarks>
		/// <param name="g">
		/// A graphic device object to be drawn into.  This is normally created with a call to
		/// the CreateGraphics() method of the Control or Form.
		/// </param>
		/// <param name="rows">The number of rows of <see c_ref="GraphPane"/> objects
		/// to include in the layout</param>
		/// <param name="columns">The number of columns of <see c_ref="GraphPane"/> objects
		/// to include in the layout</param>
		/// <seealso c_ref="SetLayout(Graphics,PaneLayout)" />
		/// <seealso c_ref="SetLayout(Graphics,bool,int[])" />
		/// <seealso c_ref="SetLayout(Graphics,bool,int[],float[])" />
		...
		/// <summary>
		/// Modify the <see c_ref="GraphPane" /> <see c_ref="PaneBase.Rect" /> sizes of each
		/// <see c_ref="GraphPane" /> such that they fit within the <see c_ref="MasterPane" />
		/// in a pre-configured layout.
		/// </summary>
		/// <remarks>The <see c_ref="SetLayout(Graphics,PaneLayout)" /> method (and overloads) is
		...
		/// A more automatic overload, using a <see c_ref="PaneLayout"/> enumeration, is available.</remarks>
		/// <param name="g">
		/// A graphic device object to be drawn into.  This is normally e.Graphics from the
		/// PaintEventArgs argument to the Paint() method.
		/// </param>
