...
{
	/// <summary>
	/// A collection class containing a list of <see c_ref="PointPair"/> objects
	...
	#region Fields
		...
		/// <see c_ref="PointPairList"/>.  Use the public property
		/// <see c_ref="Sorted"/> to access this value.
		/// </summary>
...
*/
	...
	#region Methods
		/// <summary>
		/// Add a <see c_ref="PointPair"/> object to the collection at the end of the list.
		/// </summary>
		/// <param name="point">The <see c_ref="PointPair"/> object to
		/// be added</param>
		...
		/// <summary>
		/// Add a <see c_ref="PointPairList"/> object to the collection at the end of the list.
		/// </summary>
		/// <param name="pointList">A reference to the <see c_ref="PointPairList"/> object to
		/// be added</param>
		...
		/// <summary>
		/// Add a set of points to the <see c_ref="PointPairList"/> from three arrays of type double.
		/// If the X or Y array is null, then a set of ordinal values is automatically
		/// generated in its place (see <see c_ref="AxisType.Ordinal"/>.  If the <see paramref="baseVal"/>
		...
		/// <summary>
		/// Add a single point to the <see c_ref="PointPairList"/> from values of type double.
		/// </summary>
		...
		/// <summary>
		/// Add a <see c_ref="PointPair"/> object to the collection at the specified,
		...
		/// </param>
		/// <param name="point">
		/// The <see c_ref="PointPair"/> object to be added.
		/// </param>
		...
		/// <summary>
		/// Add a single point (from values of type double ) to the <see c_ref="PointPairList"/> at the specified,
		/// zero-based, index location.
		/// </summary>
		/// <param name="index">
		/// The zero-based ordinal index where the point is to be added in the list.
		...
		/*
				/// <summary>
				/// Remove the specified <see c_ref="PointPair"/> object from the collection based
				/// the point values (must match exactly).
				/// </summary>
				/// <param name="pt">
				/// A <see c_ref="PointPair"/> that is to be removed by value.
				/// </param>
				...
				/// <see c_ref="PointPair"/> in the collection.
				/// </summary>
				/// <param name="pt">The <see c_ref="PointPair"/> object that is to be found.
				/// </param>
				/// <returns>The zero-based index of the specified <see c_ref="PointPair"/>, or -1 if the <see c_ref="PointPair"/>
				/// is not in the list</returns>
				/// <seealso c_ref="IList.IndexOf"/>
		...
		*/
		...
		/// <see c_ref="PointPair"/> with the specified label <see c_ref="PointPair.Tag"/>.
		/// </summary>
		/// <remarks>The <see c_ref="PointPair.Tag"/> object must be of type <see c_ref="String"/>
		/// for this method to find it.</remarks>
		/// <param name="label">The <see c_ref="String"/> label that is in the
		/// <see c_ref="PointPair.Tag"/> attribute of the item to be found.
		/// </param>
		/// <returns>The zero-based index of the specified <see c_ref="PointPair"/>,
		/// or -1 if the <see c_ref="PointPair"/> is not in the list</returns>
		...
		/// <summary>
		/// Compare two <see c_ref="PointPairList"/> objects to see if they are equal.
		/// </summary>
		/// <remarks>Equality is based on equal count of <see c_ref="PointPair"/> items, and
		/// each individual <see c_ref="PointPair"/> must be equal (as per the
		/// <see c_ref="PointPair.Equals"/> method.</remarks>
		/// <param name="obj">The <see c_ref="PointPairList"/> to be compared with for equality.</param>
		/// <returns>true if the <see c_ref="PointPairList"/> objects are equal, false otherwise.</returns>
		...
		/// list if the list is already sorted.
		/// </summary>
		/// <param name="type"></param>  The <see c_ref = "SortType"/>
		...
		/// <summary>
		/// Set the X values for this <see c_ref="PointPairList"/> from the specified
		/// array of double values.
		/// </summary>
		/// <remarks>
		/// If <see paramref="x"/> has more values than
		/// this list, then the extra values will be ignored.  If <see paramref="x"/>
		/// has less values, then the corresponding <see c_ref="PointPairList"/> values
		/// will not be changed.  That is, if the <see c_ref="PointPairList"/> has 20 values
		/// and <see paramref="x"/> has 15 values, then the first 15 values of the
		/// <see c_ref="PointPairList"/> will be changed, and the last 5 values will not be
		/// changed.
		/// </remarks>
		/// <param name="x">An array of double values that will replace the existing X
		/// values in the <see c_ref="PointPairList"/>.</param>
		...
		/// <summary>
		/// Set the Y values for this <see c_ref="PointPairList"/> from the specified
		/// array of double values.
		/// </summary>
		/// <remarks>
		...
		/// <summary>
		/// Set the Z values for this <see c_ref="PointPairList"/> from the specified
		/// array of double values.
		/// </summary>
		/// <remarks>
		...
		/// <summary>
		/// Add the Y values from the specified <see c_ref="PointPairList"/> object to this
		/// <see c_ref="PointPairList"/>.  If <see paramref="sumList"/> has more values than
		...
		/// </summary>
		/// <param name="sumList">A reference to the <see c_ref="PointPairList"/> object to
		...
		/// <summary>
		/// Add the X values from the specified <see c_ref="PointPairList"/> object to this
		/// <see c_ref="PointPairList"/>.  If <see paramref="sumList"/> has more values than
		/// this list, then the extra values will be ignored.  If <see paramref="sumList"/>
		/// has less values, the missing values are assumed to be zero.
		/// </summary>
		...
		/// <see c_ref="PointPairBase.Missing"/> if extrapolation would be required).  WARNING: Cardinal
		...
		/// <see c_ref="IPointList"/> instance.
		/// </summary>
		/// <remarks>The output <see c_ref="PointPairList" /> will cover the
		/// same X range of data as the original dataset.
		/// </remarks>
		/// <param name="points">An <see c_ref="IPointList" /> instance containing
		/// the data to be regressed.</param>
		/// <param name="pointCount">The number of desired points to be included
		/// in the resultant <see c_ref="PointPairList" />.
		/// </param>
		/// <returns>A new <see c_ref="PointPairList" /> containing the resultant
		/// data fit.
		/// </returns>
