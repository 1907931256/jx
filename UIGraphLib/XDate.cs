...
namespace UIGraphLib
{
	...
	#region Fields & Constants
		...
		/// <summary>
		/// The default format string to be used in <see c_ref="ToString()"/> when
...
//		public const string DefaultFormatStr = "&d-&mmm-&yy &hh:&nn";
	...
	#region Constructors
		...
		/// <summary>
		/// Construct a date class from a <see c_ref="DateTime"/> struct.
		/// </summary>
		/// <param name="dateTime">
		/// A <see c_ref="DateTime"/> struct containing the initial date information.
		/// </param>
		...
		/// <summary>
		/// Construct a date class from a calendar date and time (year, month, day, hour, minute,
		/// second), where seconds is a <see c_ref="System.Double" /> value (allowing fractional seconds). 
		/// </summary>
		/// <param name="year">An integer value for the year, e.g., 1995.</param>
		/// <param name="day">An integer value for the day of the month, e.g., 23.
		/// It is permissible to have day numbers outside of the 1-31 range,
...
*/
	...
	#region Properties
		...
		/// <summary>
		/// Returns true if this <see c_ref="XDate" /> struct is in the valid date range
		/// </summary>
	...
	#region Get/Set Date Methods
		...
		/// (see <see c_ref="XLDayMin" /> and <see c_ref="XLDayMax" />)</returns>
	...
	#region Date Conversion Methods
		...
		/// inconsistent, and prior to 45 BC the Julian Calendar did not exist.  The <see c_ref="XDate"/>
		/// struct projects only Gregorian dates backwards and does not deal with Julian calendar dates at all.  The
		/// <see c_ref="ToString(double,string)"/> method will just append a "(BC)" notation to the end of any dates
		/// prior to 1 AD, since the <see c_ref="DateTime"/> struct throws an exception when formatting earlier dates.
		/// </remarks>
		/// <param name="year">
		...
		/// </param>
		/// <param name="millisecond">
		/// The <see c_ref="System.Double" /> millisecond value (e.g., 342.5 for 342.5 milliseconds past
		/// the second).
		/// </param>
		public static void JulianDayToCalendarDate( double jDay, out int year, out int month,
	...
	#region Operator Overload Methods
		...
		/// <summary>
		/// Implicit conversion from XDate to <see c_ref="DateTime"/>.
		/// </summary>
		/// <param name="xDate">The XDate struct on which to operate</param>
		/// <returns>A <see c_ref="DateTime"/> struct representing the specified xDate value.</returns>
		...
		/// <summary>
		/// Implicit conversion from <see c_ref="DateTime"/> to <see c_ref="XDate"/>.
		/// </summary>
		/// <param name="dt">The <see c_ref="DateTime"/> struct on which to operate</param>
		/// <returns>An <see c_ref="XDate"/> struct representing the specified DateTime value.</returns>
	...
	#region General Overrides
		...
		/// <summary>
		/// Tests whether <param>obj</param> is either an <see c_ref="XDate"/> structure or
		...
		/// <summary>
		/// Compares one <see c_ref="XDate" /> object to another.
		/// </summary>
		/// <remarks>
		/// This method will throw an exception if <paramref name="target"/> is not an
		/// <see c_ref="XDate"/> object.
		/// </remarks>
		/// <param name="target">The second <see c_ref="XDate" /> object to be compared.</param>
	...
	#region String Format Conversion Methods
		...
		/// <summary>
		/// Format this XDate value using the default format string (<see c_ref="DefaultFormatStr"/>).
		/// </summary>
		/// <remarks>
		/// The formatting is done using the <see c_ref="DateTime"/> <see c_ref="System.DateTime.ToString(string)"/>
		/// method in order to provide full localization capability.  The DateTime struct is limited to
		/// dates from 1 AD onward.  However, all calendar dates in <see c_ref="XDate"/> and <see c_ref="DateTime"/>
		...
		/// </summary>
		/// <remarks>
		/// The formatting is done using the <see c_ref="DateTime" />
		/// <see c_ref="System.DateTime.ToString(String)" />
		/// method in order to provide full localization capability.  The DateTime struct is limited to
		/// dates from 1 AD onward.  However, all calendar dates in <see c_ref="XDate" /> and
		/// <see c_ref="DateTime" />
		/// are projected Gregorian calendar dates.  Since the Gregorian calendar was not implemented
		/// until October 4, 1582 (or later in some countries), Gregorian dates prior to that time are
		/// really dates that would have been, had the Gregorian calendar existed.  In order to avoid
		...
		/// string is specified according to the <see c_ref="DateTime"/> class.
		/// </summary>
		/// <remarks>
		/// The formatting is done using the <see c_ref="DateTime" />
		/// <see c_ref="System.DateTime.ToString(String)" />
		...
		/// <see c_ref="System.Globalization.DateTimeFormatInfo" />
