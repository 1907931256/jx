...
namespace UIGraphLib
{
	...
	#region ContextMenu
		...
		/// </summary>
		/// <remarks>This version is similar to the regular <see c_ref="Copy" /> method, except that
		...
		/// </summary>
		/// <remarks>
		/// Note that <see c_ref="SaveAsBitmap" /> and <see c_ref="SaveAsEmf" /> methods are provided
		...
		/// <see c_ref="SaveAs()" />, which allows for Bitmap or EMF formats
		/// </remarks>
		...
		/// See <see c_ref="SaveAsEmf()" /> for public access.
		/// </summary>
		/// <remarks>
		/// Note that this handler saves as an Emf format only.  The default handler is
		/// <see c_ref="SaveAs()" />, which allows for Bitmap or EMF formats.
		...
		/// <summary>
		/// Handler for the "Show Values" context menu item.  Toggles the <see c_ref="IsShowPointValues"/>
		...
		/// </summary>
		/// <remarks>
		/// This method differs from the <see c_ref="ZoomOutAll" /> method in that it sets the scales
		/// to full auto mode.  The <see c_ref="ZoomOutAll" /> method sets the scales to their initial
		...
		/// setting prior to any user actions (which may or may not be full auto mode).
		/// </remarks>
		/// <param name="primaryPane">The <see c_ref="GraphPane" /> object which is to have the
		...
		*/
		...
		/// </summary>
		/// <remarks>
		/// Triggers a <see c_ref="ZoomEvent" /> for any type of undo (including pan, scroll, zoom, and
		/// wheelzoom).  This method will affect all the
		/// <see c_ref="GraphPane" /> objects in the <see c_ref="MasterPane" /> if
		/// <see c_ref="IsSynchronizeXAxes" /> or <see c_ref="IsSynchronizeYAxes" /> is true.
		/// </remarks>
		/// <param name="primaryPane">The primary <see c_ref="GraphPane" /> object which is to be
		/// zoomed out</param>
		...
		/// </summary>
		/// <remarks>
		/// This method differs from the <see c_ref="RestoreScale" /> method in that it sets the scales
		/// to their initial setting prior to any user actions.  The <see c_ref="RestoreScale" /> method
		...
		/// sets the scales to full auto mode (regardless of what the initial setting may have been).
		/// </remarks>
		/// <param name="primaryPane">The <see c_ref="GraphPane" /> object which is to be zoomed out</param>
		public void ZoomOutAll( GraphPane primaryPane )
		{
			if ( primaryPane != null && !primaryPane.ZoomStack.IsEmpty )
			{
